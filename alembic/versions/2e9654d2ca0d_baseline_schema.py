...
def upgrade() -> None:
    ...
    op.create_table(
        ...
        sa.Column("roster_slots", sa.Integer(), nullable=False, server_default=sa.text("14")),
        sa.Column("starters", sa.Integer(), nullable=False, server_default=sa.text("8")),
    ...
    op.create_table(
        ...
        sa.UniqueConstraint("league_id", "name", name="uq_team_league_name"),  # <— inline UNIQUE (SQLite-safe)
    )
    ...
    op.create_table(
        ...
        sa.Column("created_at", sa.DateTime(), nullable=False),
        sa.UniqueConstraint("team_id", "symbol", name="uq_team_symbol"),  # <— inline UNIQUE (SQLite-safe)
    )
    ...
    op.create_table(
        ...
        sa.Column("created_at", sa.DateTime(), nullable=False),
        sa.UniqueConstraint("league_id", "team_id", "period", name="uq_team_score_period"),  # inline UNIQUE
    )
