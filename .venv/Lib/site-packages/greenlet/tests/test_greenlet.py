...
class TestGreenlet(TestCase):
    ...
    def _do_test_throw_to_dead_thread_doesnt_crash(self, wait_for_cleanup=False):
        ...
        if not wait_for_cleanup:
            s = str(exc.exception)
            self.assertTrue(
                s == "cannot switch to a different thread (which happens to have exited)" or "Cannot switch" in s
            )
        else:
            self.assertEqual(
                str(exc.exception),
    ...
    def test_set_parent_wrong_types(self):
        ...
        def check(glet):
            for p in None, 1, self, "42":
                ...
                self.assertEqual(
                    str(exc.exception), "GreenletChecker: Expected any type of greenlet, not " + type(p).__name__
                )
