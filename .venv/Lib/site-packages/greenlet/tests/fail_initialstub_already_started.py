...
class B(greenlet.greenlet):
    ...
    def __getattribute__(self, name):
        if name == "run" and not self.doing_it:
            ...
            self.doing_it = True
            results.append("Switch to b from B.__getattribute__ in " + type(greenlet.getcurrent()).__name__)
            b.switch()
            results.append("B.__getattribute__ back from main in " + type(greenlet.getcurrent()).__name__)
        if name == "run":
            name = "_B_run"
