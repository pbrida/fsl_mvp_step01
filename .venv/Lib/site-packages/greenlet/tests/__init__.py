...
class TestCase(unittest.TestCase, metaclass=TestCaseMetaClass):
    ...
    def wait_for_pending_cleanups(self, initial_active_threads=None, initial_main_greenlets=None):
        ...
        while (
            ...
            or (not self.expect_greenlet_leak and get_total_main_greenlets() > initial_main_greenlets)
        ):
            sleep(sleep_time)
            if time() > quit_after:
    ...
    def run_script(self, script_name, show_output=True):
        ...
        try:
            return subprocess.check_output([sys.executable, script], encoding="utf-8", stderr=subprocess.STDOUT)
