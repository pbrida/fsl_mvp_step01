...
def pytest_addoption(parser: Parser) -> None:
    ...
    group.addoption(
        ...
        help=("determines how to deal with unresolveable forward references in type " "annotations"),
    )
    add_ini_option("string")
...
def pytest_configure(config: Config) -> None:
    ...
    if fail_callback_option:
        ...
        if not callable(callback):
            raise TypeError(f"{fail_callback_option} ({qualified_name(callback.__class__)}) is not " f"a callable")
