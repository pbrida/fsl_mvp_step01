...
class Resolver(dns.resolver.BaseResolver):
    ...
    async def resolve(
        ...
        *backend*, a ``dns.asyncbackend.Backend``, or ``None``.  If ``None``,
        ...
        resolution = dns.resolver._Resolution(self, qname, rdtype, rdclass, tcp, raise_on_no_answer, search)
        if not backend:
    ...
    async def resolve_address(self, ipaddr: str, *args: Any, **kwargs: Any) -> dns.resolver.Answer:
        ...
        *ipaddr*, a ``str``, the IPv4 or IPv6 address you want to get
        ...
        return await self.resolve(dns.reversename.from_address(ipaddr), *args, **modified_kwargs)
    ...
    async def resolve_name(
        ...
        *family*, an ``int``, the address family.  If socket.AF_UNSPEC
        ...
        answers = dns.resolver.HostAnswers.make(v6=v6, v4=v4, add_empty=not raise_on_no_answer)
        if not answers:
    ...
    async def try_ddr(self, lifetime: float = 5.0) -> None:
        ...
        *lifetime*, a float, is the maximum time to spend attempting DDR.  The default
        ...
        try:
            ...
            answer = await self.resolve(dns._ddr._local_resolver_name, "svcb", lifetime=lifetime)
...
async def zone_for_name(
    ...
    while True:
        try:
            answer = await resolver.resolve(name, dns.rdatatype.SOA, rdclass, tcp, backend=backend)
