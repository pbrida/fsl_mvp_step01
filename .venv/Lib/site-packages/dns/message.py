...
class _WireReader:
    ...
    def _get_question(self, section_number, qcount):
        ...
        for _ in range(qcount):
            ...
            (rdclass, rdtype, _, _) = self.message._parse_rr_header(section_number, qname, rdclass, rdtype)
            self.message.find_rrset(section, qname, rdclass, rdtype, create=True, force_unique=True)
        ...
        for i in range(count):
            ...
            if rdtype in (dns.rdatatype.OPT, dns.rdatatype.TSIG):
                (
                    ...
                    deleting,
                    empty,
                ) = self.message._parse_special_rr_header(section_number, count, i, name, rdclass, rdtype)
            else:
...
class _TextReader:
    ...
    def _question_line(self, section_number):
        ...
        if not token.is_whitespace():
            self.last_name = self.tok.as_name(token, self.message.origin, self.relativize, self.relativize_to)
        name = self.last_name
        if name is None:
        ...
        (rdclass, rdtype, _, _) = self.message._parse_rr_header(section_number, name, rdclass, rdtype)
        self.message.find_rrset(section, name, rdclass, rdtype, create=True, force_unique=True)
        self.tok.get_eol()
    ...
    def _rr_line(self, section_number):
        ...
        # Type
        rdtype = dns.rdatatype.from_text(token.value)
        (rdclass, rdtype, deleting, empty) = self.message._parse_rr_header(section_number, name, rdclass, rdtype)
        token = self.tok.get()
            ...
            if token.is_comment():
                ...
                try:
                    ...
                    if not self.message:
                        ...
                        self.one_rr_per_rrset = message._get_one_rr_per_rrset(self.one_rr_per_rrset)
...
def from_text(
    ...
    *relativize_to*, a ``dns.name.Name`` (or ``None``), the origin to use
    ...
    reader = _TextReader(text, idna_codec, one_rr_per_rrset, origin, relativize, relativize_to)
    return reader.read()
