...
class WritableVersion(dns.zone.WritableVersion):
    ...
    def _maybe_cow_with_name(self, name: dns.name.Name) -> Tuple[dns.node.Node, dns.name.Name]:
...
class Bounds:
    ...
    def __str__(self):
        ...
        if self.is_delegation:
            ...
            zonecut = ""
        return f"{self.left} {op} {self.name} < {self.right}{zonecut}; " f"{self.closest_encloser}"
...
class ImmutableVersion(dns.zone.Version):
    def __init__(self, version: dns.zone.Version):
        if not isinstance(version, WritableVersion):
            raise ValueError("a dns.btreezone.ImmutableVersion requires a " "dns.btreezone.WritableVersion")
    ...
    def bounds(self, name: dns.name.Name | str) -> Bounds:
        ...
        *name* that is in the zone, either explicitly or by the implied existence
        ...
        if right is not None:
            ...
            right_key = None
        closest_encloser = dns.name.Name(name[-max(left_comparison[2], right_comparison[2]) :])
        return Bounds(
            name,
            left.key(),
            right_key,
...
class Zone(dns.versioned.Zone):
    ...
    writable_version_factory: Callable[[dns.zone.Zone, bool], dns.zone.Version] | None = WritableVersion
    immutable_version_factory: Callable[[dns.zone.Version], dns.zone.Version] | None = ImmutableVersion
