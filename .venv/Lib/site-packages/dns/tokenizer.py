...
class Tokenizer:
    ...
    def get_uint16(self, base: int = 10) -> int:
        ...
        if value < 0 or value > 65535:
            if base == 8:
                raise dns.exception.SyntaxError(f"{value:o} is not an octal unsigned 16-bit integer")
            else:
    ...
    def get_eol_as_token(self) -> Token:
        ...
        if not token.is_eol_or_eof():
            raise dns.exception.SyntaxError(f'expected EOL or EOF, got {token.ttype} "{token.value}"')
        return token
