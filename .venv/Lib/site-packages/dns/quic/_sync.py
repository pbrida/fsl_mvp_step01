...
class SyncQuicManager(BaseQuicManager):
    def __init__(self, conf=None, verify_mode=ssl.CERT_REQUIRED, server_name=None, h3=False):
    ...
    def connect(
        ...
        with self._lock:
            (connection, start) = self._connect(address, port, source, source_port, want_session_ticket, want_token)
            if start:
                connection.run()
            return connection
