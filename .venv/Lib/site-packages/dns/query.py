...
if _have_httpx:
    ...
    class _NetworkBackend(_CoreNetworkBackend):
        ...
        def connect_tcp(self, host, port, timeout=None, local_address=None, socket_options=None):  # pylint: disable=signature-differs
            addresses = []
            ...
            for address in addresses:
                ...
                if local_address is not None or self._local_port != 0:
                    ...
                    source = dns.inet.low_level_address_tuple((local_address, self._local_port), af)
                else:
                    source = None
                try:
...
# situations.
socket_factory: Callable[[socket.AddressFamily | int, socket.SocketKind, int], socket.socket] = default_socket_factory
...
def _matches_destination(af, from_address, destination, ignore_unexpected):
    ...
    if _addresses_equal(af, from_address, destination) or (
        ...
        return False
    raise UnexpectedSource(f"got a response from {from_address} instead of " f"{destination}")
...
def https(
    ...
    *http_version*, a ``dns.query.HTTPVersion``, indicating which HTTP version to use.
    ...
    if response.status_code < 200 or response.status_code > 299:
        raise ValueError(
            f"{where} responded with status code {response.status_code}" f"\nResponse body: {response.content}"
        )
...
def receive_udp(
    ...
    *ignore_errors* is ``True``, check that the received message is a response
    ...
    while True:
        ...
        if not _matches_destination(sock.family, from_address, destination, ignore_unexpected):
            continue
...
def udp(
    ...
    *ignore_errors*, a ``bool``.  If various format errors or response
    ...
    wire = q.to_wire()
    (af, destination, source) = _destination_and_source(where, port, source, source_port, True)
...
# for backwards compatibility
def _make_dot_ssl_context(server_hostname: str | None, verify: bool | str) -> ssl.SSLContext:
...
def xfr(
    ...
    *keyalgorithm*, a ``dns.name.Name`` or ``str``, the TSIG algorithm to use.
    ...
    if rdtype == dns.rdatatype.IXFR:
        rrset = q.find_rrset(q.authority, zone, dns.rdataclass.IN, dns.rdatatype.SOA, create=True)
