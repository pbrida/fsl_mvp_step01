...
async def _http3(
    ...
    async with cfactory() as context:
        async with mfactory(context, verify_mode=verify, server_name=hostname, h3=True) as the_manager:
            if connection:
...
async def inbound_xfr(
    ...
    *backend*, a ``dns.asyncbackend.Backend``, or ``None``.  If ``None``,
    ...
    if query.question[0].rdtype == dns.rdatatype.IXFR and udp_mode != UDPMode.NEVER:
        s = await backend.make_socket(af, socket.SOCK_DGRAM, 0, stuple, dtuple, _timeout(expiration))
        async with s:
            try:
    ...
    s = await backend.make_socket(af, socket.SOCK_STREAM, 0, stuple, dtuple, _timeout(expiration))
    async with s:
        async for _ in _inbound_xfr(  # pyright: ignore
            txn_manager,
            s,
