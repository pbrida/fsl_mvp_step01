...
class Zone(dns.transaction.TransactionManager):
    ...
    def get_node(self, name: dns.name.Name | str, create: bool = False) -> dns.node.Node | None:
    ...
    def replace_rdataset(self, name: dns.name.Name | str, replacement: dns.rdataset.Rdataset) -> None:
    ...
    def get_soa(self, txn: dns.transaction.Transaction | None = None) -> dns.rdtypes.ANY.SOA.SOA:
        ...
        for name, node in sorted(self.items()):
            ...
            for rdataset in sorted(node, key=lambda rds: (rds.rdtype, rds.covers)):
                if name == origin_name and dns.rdatatype.ZONEMD in (
                    ...
                    continue
                rrfixed = struct.pack("!HHI", rdataset.rdtype, rdataset.rdclass, rdataset.ttl)
...
class ImmutableVersion(Version):
    def __init__(self, version: Version):
        if not isinstance(version, WritableVersion):
            raise ValueError("a dns.zone.ImmutableVersion requires a dns.zone.WritableVersion")
