...
class NoNameservers(dns.exception.DNSException):
    ...
    def _fmt_kwargs(self, **kwargs):
        srv_msgs = _errors_to_text(kwargs["errors"])
        return super()._fmt_kwargs(query=kwargs["request"].question, errors="; ".join(srv_msgs))
...
class _Resolution:
    ...
    def query_result(self, response: dns.message.Message | None, ex: Exception | None) -> Tuple[Answer | None, bool]:
        #
...
class BaseResolver:
    ...
    def _get_qnames_to_try(self, qname: dns.name.Name, search: bool | None) -> List[dns.name.Name]:
    ...
    def _enrich_nameservers(
        ...
        if isinstance(nameservers, list | tuple):
            ...
            raise ValueError(f"nameservers must be a list or tuple (not a {type(nameservers)})")
...
def query(
    ...
    further details.
    """
    warnings.warn("please use dns.resolver.resolve() instead", DeprecationWarning, stacklevel=2)
    return resolve(
        qname,
        rdtype,
        rdclass,
...
def resolve_name(name: dns.name.Name | str, family: int = socket.AF_UNSPEC, **kwargs: Any) -> HostAnswers:
    """Use a resolver to query for address records.
    ...
    while 1:
        try:
            ...
            if expiration is not None:
                ...
                rlifetime = None
            answer = resolver.resolve(name, dns.rdatatype.SOA, rdclass, tcp, lifetime=rlifetime)
