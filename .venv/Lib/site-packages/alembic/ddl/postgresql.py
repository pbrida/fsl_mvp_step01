...
from sqlalchemy import Column, Float, Identity, Numeric, literal_column, select, text
...
class PostgresqlImpl(DefaultImpl):
    ...
    def _render_HSTORE_type(self, type_: HSTORE, autogen_context: AutogenContext) -> str:
        return cast(
            str,
            render._render_type_w_subtype(type_, autogen_context, "text_type", r"(.+?\(.*text_type=)"),
        )
    ...
    def _render_JSON_type(self, type_: JSON, autogen_context: AutogenContext) -> str:
        return cast(
            str,
            render._render_type_w_subtype(type_, autogen_context, "astext_type", r"(.+?\(.*astext_type=)"),
        )
    ...
    if element.default is None:
        ...
        for attr in sorted(diff):
            if attr == "always":
                ...
                text += "SET %s " % compiler.get_identity_options(Identity(**{attr: getattr(identity, attr)}))
        return text
