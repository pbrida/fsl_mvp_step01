...
import logging
import re
from typing import TYPE_CHECKING, Any, Dict, Iterator, Mapping, Optional, Set, Tuple, TypeVar, Union, cast
    ...
    for s, tname in conn_table_names.difference(metadata_table_names):
        ...
        if not exists:
            event.listen(
                ...
                "column_reflect",
                # fmt: off
                autogen_context.migration_context.impl._compat_autogen_column_reflect(inspector),
                # fmt: on
            )
...
def _compare_indexes_and_uniques(
    ...
    if conn_table is not None:
        ...
        conn_indexes = {  # type:ignore[assignment]
            index for index in (_make_index(impl, ix, conn_table) for ix in conn_indexes) if index is not None
        }
    ...
    conn_names = {
        c.name: c for c in conn_unique_constraints.union(conn_indexes_sig) if sqla_compat.constraint_name_string(c.name)
    }
...
def _correct_for_uq_duplicates_uix(
    ...
    unnamed_metadata_uqs = {
        impl._create_metadata_constraint_sig(cons).unnamed for name, cons in metadata_cons_names if name is None
    }
...
def _compare_server_default(
    ...
    if sqla_compat._server_default_is_identity(metadata_default, conn_col_default):
        ...
        rendered_metadata_default = _render_server_default_for_compare(metadata_default, autogen_context)
