...
def _add_table(autogen_context: AutogenContext, op: ops.CreateTableOp) -> str:
    ...
    args = [col for col in [_render_column(col, autogen_context) for col in table.columns] if col] + sorted(
        [
            rcons
            for rcons in [
                _render_constraint(cons, autogen_context, op._namespace_metadata) for cons in table.constraints
            ]
...
def _render_dialect_kwargs_items(autogen_context: AutogenContext, dialect_kwargs: _DialectArgView) -> list[str]:
    return [f"{key}={_render_potential_expr(val, autogen_context)}" for key, val in dialect_kwargs.items()]
    ...
    return "%(prefix)sColumn(%(name)r, %(type)s, %(args)s%(kwargs)s)" % {
        ...
        "kwargs": (
            ", ".join(
                ...
                + ["%s=%s" % (key, _render_potential_expr(val, autogen_context)) for key, val in column.kwargs.items()]
            )
        ),
    }
...
def _render_foreign_key(
    ...
    return "%(prefix)sForeignKeyConstraint([%(cols)s], " "[%(refcols)s], %(args)s)" % {
        ...
        "refcols": ", ".join(
            repr(_fk_colspec(f, apply_metadata_schema, namespace_metadata)) for f in constraint.elements
        ),
...
def _render_check_constraint(
    ...
    return "%(prefix)sCheckConstraint(%(sqltext)s%(opts)s)" % {
        ...
        "sqltext": _render_potential_expr(constraint.sqltext, autogen_context, wrap_in_element=False),
    }
