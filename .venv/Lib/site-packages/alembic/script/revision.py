...
class RevisionMap:
    ...
    def _normalize_depends_on(self, revisions: Collection[Revision], map_: _RevisionMapType) -> None:
                ...
                try:
                    ...
                    if rint < 0:
                        ...
                        if branch_label is not None:
                            select_heads = tuple(
                                head for head in select_heads if branch_label in is_revision(head).branch_labels
                            )
    ...
    def filter_for_lineage(
        ...
        return tuple(
            tg for tg in targets if self._shares_lineage(tg, shares, include_dependencies=include_dependencies)
        )
    ...
    def _resolve_revision_number(self, id_: Optional[_GetRevArg]) -> Tuple[Tuple[str, ...], Optional[str]]:
        ...
        if isinstance(id_, str) and "@" in id_:
            ...
            (isinstance(id_, tuple) and id_ and not isinstance(id_[0], str)) or not isinstance(id_, (str, tuple))
        ):
            raise RevisionError(
    ...
    def _topological_sort(
        ...
        while current_heads:
            ...
            for check_head_index, ancestors in enumerate(ancestors_by_idx):
                ...
                heads_to_add = [
                    r for r in candidate_rev._normalized_down_revisions if r in todo and r not in current_heads
                ]
    ...
    def _walk(
        ...
        for _ in range(abs(steps)):
            if steps > 0:
                ...
                # Walk up
                walk_up = [
                    is_revision(rev) for rev in self.get_revisions(self.bases if initial is None else initial.nextrev)
                ]
                if branch_label:
                ...
                if initial == "base":  # type: ignore[comparison-overlap]
                    ...
                    children = self.get_revisions(self.heads if initial is None else initial.down_revision)
                    if not children:
                        children = ("base",)
            if not children:
        ...
        if match:
            ...
            if rel_int >= 0:
                if symbol is None:
                    ...
                    raise RevisionError(
                        "Relative revision %s didn't " "produce %d migrations" % (relative, abs(rel_int))
                    )
    ...
    def _parse_upgrade_target(
        ...
        if relative > 0:
            if symbol is None:
                ...
                if branch_label:
                    ...
                    if not start_revs:
                        ...
                        start_revs = tuple(
                            ...
                            - {down for rev in active_on_branch for down in rev._normalized_down_revisions}
                        )
                        if not start_revs:
                ...
                if rev is None:
                    raise RevisionError(
                        "Relative revision %s didn't " "produce %d migrations" % (relative_str, abs(relative))
                    )
                return (rev,)
            else:
    ...
    def _collect_downgrade_revisions(
        ...
        if branch_label and len(roots) > 1:
            ...
            roots = [
                is_revision(rev) for rev in self.get_revisions({rev.revision for rev in roots}.intersection(ancestors))
            ]
...
class Revision:
    ...
    def _normalized_down_revisions(self) -> Tuple[str, ...]:
        ...
        """
        return util.dedupe_tuple(util.to_tuple(self.down_revision, default=()) + self._normalized_resolved_dependencies)
