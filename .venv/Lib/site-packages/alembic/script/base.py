...
from typing import TYPE_CHECKING, Any, Iterator, List, Optional, Sequence, Set, Tuple, Union, cast
...
class ScriptDirectory:
    ...
    def __init__(
        ...
        if not os.access(dir, os.F_OK):
            raise util.CommandError(
                f"Path doesn't exist: {dir}.  Please use " "the 'init' command to create a new " "scripts folder."
            )
    ...
    def _version_locations(self) -> Sequence[Path]:
        if self.version_locations:
            return [util.coerce_resource_to_filename(location).absolute() for location in self.version_locations]
        else:
        ...
        for vers in paths:
            for file_path in Script._list_py_dir(self, vers):
                ...
                if real_path in dupes:
                    util.warn(f"File {real_path} loaded twice! ignoring. " "Please ensure version_locations is unique.")
                    continue
                dupes.add(real_path)
    ...
    def generate_revision(
        ...
        **kw: Any,
        ...
        for vers_path in self._version_locations:
            ...
            raise util.CommandError(f"Path {version_path} is not represented in current " "version locations")
        ...
        if depends_on:
            with self._catch_revision_errors():
                resolved_depends_on = [
                    ...
                    for rev, dep in [
                        (not_none(self.revision_map.get_revision(dep)), dep) for dep in util.to_list(depends_on)
                    ]
                ]
        else:
...
class Script(revision.Revision):
        ...
        for root, dirs, files in compat.path_walk(path, top_down=True):
            ...
            if scriptdir.sourceless:
                ...
                if py_cache_path.exists():
                    ...
                    paths.extend(
                        py_cache_path / pyc for pyc in py_cache_path.iterdir() if pyc.name.split(".")[0] not in names
                    )
    ...
    @classmethod
    def _from_path(cls, scriptdir: ScriptDirectory, path: Union[str, os.PathLike[str]]) -> Optional[Script]:
        path = Path(path)
