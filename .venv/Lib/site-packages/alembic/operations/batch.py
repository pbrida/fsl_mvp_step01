...
class ApplyBatchImpl:
    ...
    def _gather_indexes_from_both_tables(self) -> List[Index]:
        ...
        for idx_existing in self.indexes.values():
            ...
            idx_copy = Index(
                ...
                *[_copy_expression(expr, self.new_table) for expr in _idx_table_bound_expressions(idx_existing)],
    ...
    def alter_column(
        ...
        **kw,
        ...
        if name is not None and name != column_name:
            ...
            if existing_type:
                resolved_existing_type = _resolve_for_variant(kw["existing_type"], self.impl.dialect)

                # pop named constraints for Boolean/Enum for rename
                if (
                    isinstance(resolved_existing_type, SchemaEventTarget) and resolved_existing_type.name  # type:ignore[attr-defined]  # noqa E501
                ):
    ...
    def drop_column(
        ...
        **kw,
        ...
        # pop named constraints for Boolean/Enum for rename
        if (
            "existing_type" in kw and isinstance(kw["existing_type"], SchemaEventTarget) and kw["existing_type"].name  # type:ignore[attr-defined]
        ):
            self.named_constraints.pop(
