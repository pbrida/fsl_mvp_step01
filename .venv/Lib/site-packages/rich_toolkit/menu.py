...
class Menu(Generic[ReturnValue], TextInputHandler, Element):
    ...
    def get_key(self) -> Optional[str]:
        ...
        next_keys, prev_keys = (right_keys, left_keys) if self.inline else (down_keys, up_keys)
    ...
    def options(self) -> List[Option[ReturnValue]]:
        if self.allow_filtering:
            return [option for option in self._options if self.text.lower() in option["name"].lower()]
    ...
    def handle_key(self, key: str) -> None:
        ...
        if current_selection:
            matching_index = next(
                (index for index, option in enumerate(self.options) if option["name"] == current_selection),
                0,
            )
