...
class TaggedStyle(BaseStyle):
    ...
    def _get_tag_segments(
        ...
        if is_animated:
            animation_status: Literal["started", "stopped", "error"] = "started" if not done else "stopped"

            tag = " " * self.block_length
            colors = self._get_animation_colors(steps=self.block_length, animation_status=animation_status)
            ...
            tag_segments = [
                Segment(
                    self.block,
                    style=Style(color=colors[(self.animation_counter + i) % len(colors)]),
                )
    ...
    def render_element(
        ...
        **kwargs: Any,
        ...
        rendered = super().render_element(element=element, is_active=is_active, done=done, parent=parent, **kwargs)
