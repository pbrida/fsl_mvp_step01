...
class FancyPanel:
    ...
    def _get_decoration(self, suffix: str = "") -> Segment:
        ...
        animation_status: Literal["started", "stopped", "error"] = "started" if animated else "stopped"

        color = self.style._get_animation_colors(steps=14, breathe=True, animation_status=animation_status)[
            self.counter % 14
        ]

        return Segment(char + suffix, style=Style.from_color(color))

    def _strip_trailing_newlines(self, lines: List[List[Segment]]) -> List[List[Segment]]:
    ...
    def __rich_console__(self, console: "Console", options: "ConsoleOptions") -> "RenderResult":
        ...
        for first, last, line in loop_first_last(lines):
            if first and not self._title:
                decoration = Segment("┌ ") if self.metadata.get("title", False) else self._get_decoration(suffix=" ")
...
class FancyStyle(BaseStyle):
    ...
    def render_element(
        ...
        **metadata: Any,
        ...
        rendered = super().render_element(element=element, is_active=is_active, done=done, parent=parent, **metadata)
