    ...
    def process_redirect(self, exc: Exception) -> Exception | str:
        ...
        if (
            ...
            if self.connection_kwargs.get("unix", False):
                return ValueError(f"cannot follow cross-origin redirect to {new_uri} " f"with a Unix socket")

            # Cross-origin redirects when host and port are overridden are ill-defined.
            if self.connection_kwargs.get("host") is not None or self.connection_kwargs.get("port") is not None:
                return ValueError(f"cannot follow cross-origin redirect to {new_uri} " f"with an explicit host or port")
