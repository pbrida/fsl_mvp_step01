...
class ColumnElement(
    ...
    def comparator(self) -> TypeEngine.Comparator[_T]:
        try:
            ...
            raise TypeError(
                "Object %r associated with '.type' attribute " "is not a TypeEngine class or object" % self.type
            ) from err
        else:
...
class TextClause(
        ...
        for bind in binds:
            try:
                ...
                raise exc.ArgumentError(
                    "This text() construct doesn't define a " "bound parameter named %r" % bind._orig_key
                ) from err
            else:
    ...
    def columns(self, *cols: _ColumnExpressionArgument[Any], **types: TypeEngine[Any]) -> TextualSelect:
        ...
        positional_input_cols = [
            (ColumnClause(col.key, types.pop(col.key)) if col.key in types else col) for col in input_cols
        ]
    ...
    def __init__(
        ...
        *clauses: _ColumnExpressionArgument[Any],
        ...
        if self.group_contents:
            self.clauses = [
                coercions.expect(text_converter_role, clause, apply_propagate_attrs=self).self_group(
            ...
            self.clauses = [
                coercions.expect(text_converter_role, clause, apply_propagate_attrs=self) for clause in clauses_iterator
            ]
    ...
    def self_group(self, against: Optional[OperatorType] = None) -> Union[Self, Grouping[_T]]:
        if (
            ...
            or (
                ...
                # group for that
                against is operators.inv and not operators.is_boolean(self.operator)
            )
        ):
            return Grouping(self)
        else:
...
class ExpressionClauseList(OperatorExpression[_T]):
    ...
    def __init__(
        ...
        *clauses: _ColumnExpressionArgument[Any],
        ...
        self.clauses = tuple(
            coercions.expect(roles.ExpressionElementRole, clause, apply_propagate_attrs=self) for clause in clauses
        )
        self._is_implicitly_boolean = operators.is_boolean(self.operator)
...
class BooleanClauseList(ExpressionClauseList[bool]):
    ...
    def and_(
        cls,
        initial_clause: Union[Literal[True], _ColumnExpressionArgument[bool], _NoArg] = _NoArg.NO_ARG,
    ...
    def or_(
        cls,
        initial_clause: Union[Literal[False], _ColumnExpressionArgument[bool], _NoArg] = _NoArg.NO_ARG,
        *clauses: _ColumnExpressionArgument[bool],
    ) -> ColumnElement[bool]:
...
class Tuple(ClauseList, ColumnElement[typing_Tuple[Any, ...]]):
    ...
    def __init__(
        ...
        *clauses: _ColumnExpressionArgument[Any],
        ...
        if types is None:
            init_clauses: List[ColumnElement[Any]] = [coercions.expect(roles.ExpressionElementRole, c) for c in clauses]
        else:
            if len(types) != len(clauses):
                raise exc.ArgumentError("Wrong number of elements for %d-tuple: %r " % (len(types), clauses))
            init_clauses = [
                coercions.expect(
...
class Case(ColumnElement[_T]):
    ...
    def __init__(
        ...
        *whens: Union[
        ...
        new_whens: Iterable[Any] = coercions._expression_collection_was_a_list("whens", "case", whens)
        try:
...
class _OverrideBinds(Grouping[_T]):
    ...
    def __init__(
        ...
        self.element = element
        self.translate = {k.key: v.value for k, v in zip(replaces_params, bindparams, strict=False)}
...
class Over(ColumnElement[_T]):
    ...
    def _from_objects(self) -> List[FromClause]:
        return list(
            itertools.chain(
                *[c._from_objects for c in (self.element, self.partition_by, self.order_by) if c is not None]
            )
        )
...
class FunctionFilter(ColumnElement[_T]):
    ...
    @util.ro_non_memoized_property
    def _from_objects(self) -> List[FromClause]:
        return list(itertools.chain(*[c._from_objects for c in (self.func, self.criterion) if c is not None]))
...
class Label(roles.LabeledColumnExprRole[_T], NamedColumn[_T]):
    ...
    def _copy_internals(
        ...
        **kw: Any,
        ...
        if anonymize_labels:
            self.name = _anonymous_label.safe_construct(id(self), getattr(self.element, "name", "anon"))
            self.key = self._tq_label = self._tq_key_label = self.name
...
class ColumnClause(
    ...
    def _compare_name_for_result(self, other):
        if (
            ...
            or (
                isinstance(other, ColumnClause) and (other.is_literal or other.table is None or other.table._is_textual)
            )
        ):
            return (hasattr(other, "name") and self.name == other.name) or (
