...
from .visitors import ClauseVisitor, ExtendedInternalTraversal, ExternallyTraversible, InternalTraversal
...
def _generative(fn: _Fn) -> _Fn:
    ...
    @util.decorator
    def _generative(fn: _Fn, self: _SelfGenerativeType, *args: Any, **kw: Any) -> _SelfGenerativeType:
    ...
    def check(fn, *args, **kw):
        ...
        for name, getter, default_ in getters:
            if getter(self) is not default_:
                msg = msgs.get(
                    name,
                    "Method %s() has already been invoked on this %s construct" % (fn.__name__, self.__class__),
                )
    ...
    def argument_for(cls, dialect_name, argument_name, default):
        ...
        if construct_arg_dictionary is None:
            raise exc.ArgumentError(
                "Dialect '%s' does have keyword-argument " "validation and defaults enabled configured" % dialect_name
            )
    ...
    def _validate_dialect_kwargs(self, kwargs: Dict[str, Any]) -> None:
        ...
        for k in kwargs:
            ...
            try:
                ...
                util.warn(
                    "Can't validate argument %r; can't " "locate any SQLAlchemy dialect named %r" % (k, dialect_name)
                )
...
class Options(metaclass=_MetaOptions):
    ...
    def __init_subclass__(cls) -> None:
        ...
        cls._cache_attrs = tuple(
            sorted(d for d in dict_ if not d.startswith("__") and d not in ("_cache_key_traversal",))
        )
    ...
    def __repr__(self):
        ...
        return "%s(%s)" % (
            ...
            ", ".join("%s=%r" % (k, self.__dict__[k]) for k in self._cache_attrs if k in self.__dict__),
        )
    ...
    def safe_merge(cls, other):
        ...
        if cls is not other.__class__ and other._cache_attrs and set(other._cache_attrs).difference(cls._cache_attrs):
            raise TypeError(
    ...
    def contains_column(self, col: ColumnElement[Any]) -> bool:
        ...
        if col not in self._colset:
            if isinstance(col, str):
                raise exc.ArgumentError(
                    "contains_column cannot be used with string arguments. " "Use ``col_name in table.c`` instead."
                )
            return False
        else:
            return True
