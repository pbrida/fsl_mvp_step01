...
from .visitors import ExternallyTraversible, InternalTraversal

if typing.TYPE_CHECKING:
    from ..engine import Connection, Engine
    from ..engine.interfaces import CoreExecuteOptionsParameter, ExecutionContext, _CoreMultiExecuteParams
    ...
    from .visitors import _TraverseInternalsType, anon_map
            ...
            if item is not None:
                try:
                    ...
                    raise exc.ArgumentError(
                        "'SchemaItem' object, such as a 'Column' or a " f"'Constraint' expected, got {item!r}"
                    ) from err
                else:
                    spwd(self, **kw)
...
class Table(DialectKWArgs, HasSchemaAttr, TableClause, inspection.Inspectable["Table"]):
    ...
    def _new(cls, *args: Any, **kw: Any) -> Any:
        ...
        try:
            ...
            raise TypeError("Table() takes at least two positional-only " "arguments 'name' and 'metadata'")
            ...
            if the_sentinel_zero.identity:
                if the_sentinel_zero.identity._increment_is_negative:
                    if sentinel_is_explicit:
                        raise exc.InvalidRequestError(
                            "Can't use IDENTITY default with negative " "increment as an explicit sentinel column"
                        )
                    else:
                ...
                if the_sentinel_zero.default.is_sentinel:
                    ...
                    if the_sentinel_zero.default._increment_is_negative:
                        if sentinel_is_explicit:
                            raise exc.InvalidRequestError(
                                "Can't use SEQUENCE default with negative " "increment as an explicit sentinel column"
                            )
                        else:
                            if sentinel_is_autoinc:
                                autoinc_col = None
...
class Column(DialectKWArgs, SchemaItem, ColumnClause[_T]):
    ...
    def _set_parent(  # type: ignore[override]
        ...
        **kw: Any,
        ...
        if self.index:
            ...
            table.append_constraint(Index(None, self.key, unique=bool(self.unique), _column_flag=True))
        ...
        if self.identity and (isinstance(self.default, Sequence) or isinstance(self.onupdate, Sequence)):
    ...
    @util.deprecated(
        "1.4",
        "The :meth:`_schema.Column.copy` method is deprecated " "and will be removed in a future release.",
    )
    ...
    def _merge(self, other: Column[Any]) -> None:
        ...
        if self._user_defined_nullable is not NULL_UNSPECIFIED and other._user_defined_nullable is NULL_UNSPECIFIED:
    ...
    def _make_proxy(
        ...
        **kw: Any,
        ...
        if name is None and self.name is None:
            raise exc.InvalidRequestError(
                "Cannot initialize a sub-selectable" " with this Column object until its 'name' has " "been assigned."
            )
        try:
            c = self._constructor(
                (
...
class ForeignKey(DialectKWArgs, SchemaItem):
        ...
        **dialect_kw: Any,
        ...
        if isinstance(self._colspec, str):
            ...
            if not isinstance(self._table_column.table, (type(None), TableClause)):
                raise exc.ArgumentError(
                    "ForeignKey received Column not bound " "to a Table, got: %r" % self._table_column.table
                )
    ...
    @util.deprecated(
        "1.4",
        "The :meth:`_schema.ForeignKey.copy` method is deprecated " "and will be removed in a future release.",
    )
...
class _InsertSentinelColumnDefault(ColumnDefault):
    ...
    def _set_parent(self, parent: SchemaEventTarget, **kw: Any) -> None:
        ...
        if not col._insert_sentinel:
            raise exc.ArgumentError(
                "The _InsertSentinelColumnDefault may only be applied to a " "Column marked as insert_sentinel=True"
            )
        elif not col.nullable:
            raise exc.ArgumentError(
                "The _InsertSentinelColumnDefault may only be applied to a " "Column that is nullable"
            )
...
class Constraint(DialectKWArgs, HasConditionalDDL, SchemaItem):
    ...
    def _should_create_for_compiler(self, compiler: DDLCompiler, **kw: Any) -> bool:
        if self._create_rule is not None and not self._create_rule(compiler):
            ...
            return self._ddl_if._should_execute(ddl.CreateConstraint(self), self, None, compiler=compiler, **kw)
        else:
            return True
    ...
    def table(self) -> Table:
        try:
            ...
            pass
        raise exc.InvalidRequestError(
            "This constraint is not bound to a table.  Did you " "mean to call table.append_constraint(constraint) ?"
        )
    ...
    @util.deprecated(
        "1.4",
        "The :meth:`_schema.Constraint.copy` method is deprecated " "and will be removed in a future release.",
    )
    ...
    def _check_attach(self, evt: bool = False) -> None:
        ...
        if len(tables) == 1:
            ...
            if others:
                ...
                raise exc.ArgumentError(f"Column(s) {other_str} " f"are not part of table '{table.description}'.")
...
class CheckConstraint(ColumnCollectionConstraint):
    ...
    @util.deprecated(
        "1.4",
        "The :meth:`_schema.CheckConstraint.copy` method is deprecated " "and will be removed in a future release.",
    )
        ...
        **dialect_kw: Any,
        ...
        if len(set(columns)) != len(refcolumns):
            if len(set(columns)) != len(columns):
                ...
                raise exc.ArgumentError(
                    "ForeignKeyConstraint with duplicate source column " "references are not supported."
                )
            else:
        ...
        fkc = ForeignKeyConstraint(
            ...
            [
                x._get_colspec(
                    schema=schema,
                    table_name=(
                        target_table.name if target_table is not None and x._table_key() == x.parent.table.key else None
                    ),
                    _is_copy=True,
                )
    ...
    def _autoincrement_column(self) -> Optional[Column[int]]:
        def _validate_autoinc(col: Column[Any], autoinc_true: bool) -> bool:
            if col.type._type_affinity is None or not issubclass(
                ...
                if autoinc_true:
                    raise exc.ArgumentError(
                        f"Column type {col.type} on column '{col}' is not " f"compatible with autoincrement=True"
                    )
                else:
                    return False
...
class Index(DialectKWArgs, ColumnCollectionMixin, HasConditionalDDL, SchemaItem):
    ...
    def __repr__(self) -> str:
        ...
        return "Index(%s)" % (
            ", ".join([repr(self.name)] + [repr(e) for e in self.expressions] + (self.unique and ["unique=True"] or []))
        )
...
DEFAULT_NAMING_CONVENTION: _NamingSchemaParameter = util.immutabledict({"ix": "ix_%(column_0_label)s"})
...
class MetaData(HasSchemaAttr):
    ...
    def __init__(
        ...
        if schema is not None and not isinstance(schema, str):
            raise exc.ArgumentError("expected schema argument to be a string, " f"got {type(schema)}.")
        self.tables = util.FacadeDict()
        self.schema = quoted_name.construct(schema, quote_schema)
        self.naming_convention = naming_convention if naming_convention else DEFAULT_NAMING_CONVENTION
        if info:
            self.info = info
        self._schemas: Set[str] = set()
        self._sequences: Dict[str, Sequence] = {}
        self._fk_memos: Dict[Tuple[str, Optional[str]], List[ForeignKey]] = collections.defaultdict(list)
    ...
    def reflect(
        ...
        **dialect_kwargs: Any,
        ...
        with inspection.inspect(bind)._inspection_context() as insp:
            ...
            if schema is not None:
                available_w_schema: util.OrderedSet[str] = util.OrderedSet([f"{schema}.{name}" for name in available])
            else:
...
class Computed(FetchedValue, SchemaItem):
    ...
    def _set_parent(self, parent: SchemaEventTarget, **kw: Any) -> None:
        ...
        if not isinstance(parent.server_default, (type(None), Computed)) or not isinstance(
            ...
            raise exc.ArgumentError(
                "A generated column cannot specify a server_default or a " "server_onupdate argument"
            )
        self.column = parent
    ...
    @util.deprecated(
        "1.4",
        "The :meth:`_schema.Computed.copy` method is deprecated " "and will be removed in a future release.",
    )
...
class Identity(IdentityOptions, FetchedValue, SchemaItem):
    ...
    def _set_parent(self, parent: SchemaEventTarget, **kw: Any) -> None:
        ...
        if not isinstance(parent.server_default, (type(None), Identity)) or not isinstance(
            ...
            raise exc.ArgumentError(
                "A column with an Identity object cannot specify a " "server_default or a server_onupdate argument"
            )
        if parent.autoincrement is False:
            raise exc.ArgumentError("A column with an Identity object cannot specify " "autoincrement=False")
        self.column = parent
    ...
    @util.deprecated(
        "1.4",
        "The :meth:`_schema.Identity.copy` method is deprecated " "and will be removed in a future release.",
    )
