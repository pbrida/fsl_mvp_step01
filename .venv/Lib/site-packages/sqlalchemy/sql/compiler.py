...
from . import base, coercions, crud, elements, functions, operators, roles, schema, selectable, sqltypes
from . import util as sql_util
from ._typing import is_column_element, is_dml
from .base import _NONE_NAME, NO_ARG, Executable, _de_clone, _from_objects, _SentinelDefaultCharacterization
...
class SQLCompiler(Compiled):
    ...
    def __init__(
        ...
        **kwargs: Any,
        ...
        if self.positional:
            ...
            if nb:
                self._numeric_binds_identifier_char = "$" if dialect.paramstyle == "numeric_dollar" else ":"
        ...
        if self.isinsert or self.isupdate or self.isdelete:
            ...
            if self.isinsert or self.isupdate:
                ...
                if statement._inline:
                    ...
                    not self.isinsert or (self.dialect.insert_executemany_returning and statement._return_defaults)
                ):
                    self.inline = True
    ...
    def _process_numeric(self):
        ...
        if self._insertmanyvalues:
            single_values_expr = (
                ...
                # place-holders
                self._insertmanyvalues.single_values_expr % param_pos
            )
    ...
    def construct_params(
        ...
        if params:
            ...
            for bindparam, name in self.bind_names.items():
                escaped_name = self.escaped_bind_names.get(name, name) if has_escaped_names else name
                ...
                if not parameter.literal_execute:
                    ...
                    if parameter.type._is_tuple_type:
                        ...
                        new_processors.update(
                            ...
                            if name in tuple_processors and tuple_processors[name][j - 1] is not None
                        )
                    else:
                        new_processors.update(
                            (key, single_processors[name]) for key, _ in to_update if name in single_processors
                        )
    ...
    def _inserted_primary_key_from_lastrowid_getter(self):
        ...
        def get(lastrowid, parameters):
            ...
            if lastrowid is None:
                ...
                return row_fn(
                    (
                        (autoinc_getter(lastrowid, parameters) if autoinc_getter is not None else lastrowid)
    ...
    def _inserted_primary_key_from_returning_getter(self):
        ...
        def get(row, parameters):
            return row_fn(getter(row) if use_row else getter(parameters) for getter, use_row in getters)
    ...
    def visit_column(
        ...
        **kwargs: Any,
        ...
        if table is None or not include_table or not table.named_with_column:
            ...
            if not effective_schema and ambiguous_table_name_map and tablename in ambiguous_table_name_map:
    ...
    def _generate_delimited_and_list(self, clauses, **kw):
        ...
        if lcc == 1:
            ...
            return separator.join(s for s in (c._compiler_dispatch(self, **kw) for c in clauses) if s)
        ...
        if disp:
            ...
            if name:
                ...
                name = (
                    ...
                    if self.preparer._requires_quotes_illegal_chars(name) or isinstance(name, elements.quoted_name)
                    else name
                )
                name = name + "%(expr)s"
            text = ".".join(
                [
                    (
                        ...
                        if self.preparer._requires_quotes_illegal_chars(tok) or isinstance(name, elements.quoted_name)
                        else tok
                    )
    ...
    def visit_sequence(self, sequence, **kw):
        raise NotImplementedError("Dialect '%s' does not support sequence increments." % self.dialect.name)
    ...
    def visit_compound_select(self, cs, asfrom=False, compound_index=None, **kwargs):
        ...
        entry = self._default_stack_entry if toplevel else self.stack[-1]
        need_result_map = toplevel or (not compound_index and entry.get("need_result_map_for_compound", False))
        ...
        text = (" " + keyword + " ").join(
            (c._compiler_dispatch(self, asfrom=asfrom, compound_index=i, **kwargs) for i, c in enumerate(cs.selects))
        )
    ...
    def visit_floordiv_binary(self, binary, operator, **kw):
        if self.dialect.div_is_floordiv and binary.right.type._type_affinity is sqltypes.Integer:
            ...
            return "FLOOR(%s)" % (self.process(binary.left, **kw) + " / " + self.process(binary.right, **kw))
    ...
    def visit_empty_set_expr(self, element_types, **kw):
        raise NotImplementedError("Dialect '%s' does not support empty set expression." % self.dialect.name)
    ...
    def _literal_execute_expanding_parameter_literal_binds(self, parameter, values, bind_expression_template=None):
        ...
        if not values:
            ...
            replacement_expression = ("VALUES " if self.dialect.tuple_in_values else "") + ", ".join(
                "(%s)"
                % (
                    ", ".join(
                        self.render_literal_value(value, param_type)
                        for value, param_type in zip(tuple_element, parameter.type.types, strict=False)
                    )
                )
    ...
    def _literal_execute_expanding_parameter(self, name, parameter, values):
        ...
        if not values:
            to_update = []
            if typ_dialect_impl._is_tuple_type:
                replacement_expression = self.visit_empty_set_op_expr(parameter.type.types, parameter.expand_op)
            else:
                replacement_expression = self.visit_empty_set_op_expr([parameter.type], parameter.expand_op)
            ...
            replacement_expression = ", ".join(_render_bindtemplate(key) for key, value in to_update)
    ...
    def visit_custom_op_unary_operator(self, element, operator, **kw):
        return self._generate_generic_unary_operator(element, self.escape_literal_column(operator.opstring) + " ", **kw)

    def visit_custom_op_unary_modifier(self, element, operator, **kw):
        return self._generate_generic_unary_modifier(element, " " + self.escape_literal_column(operator.opstring), **kw)
    ...
    def visit_between_op_binary(self, binary, operator, **kw):
        ...
        return self._generate_generic_binary(binary, " BETWEEN SYMMETRIC " if symmetric else " BETWEEN ", **kw)
    ...
    def visit_regexp_match_op_binary(self, binary, operator, **kw):
        raise exc.CompileError("%s dialect does not support regular expressions" % self.dialect.name)

    def visit_not_regexp_match_op_binary(self, binary, operator, **kw):
        raise exc.CompileError("%s dialect does not support regular expressions" % self.dialect.name)

    def visit_regexp_replace_op_binary(self, binary, operator, **kw):
        raise exc.CompileError("%s dialect does not support regular expression replacements" % self.dialect.name)
    ...
    def visit_bindparam(
        ...
        **kwargs,
        ...
        if not literal_binds:
            literal_execute = (
                literal_execute or bindparam.literal_execute or (within_columns_clause and self.ansi_bind_rules)
            )
        ...
        if name in self.binds:
            ...
            if existing is not bindparam:
                if (
                    ...
                    raise exc.CompileError(
                        "Bind parameter '%s' conflicts with " "unique bind parameter of the same name" % name
                    )
    ...
    def _dispatch_independent_ctes(self, stmt, kw):
        ...
        for cte, opt in zip(stmt._independent_ctes, stmt._independent_ctes_opts, strict=False):
        ...
        **kwargs: Any,
        ...
        if existing_cte is not None:
            ...
            if cte is existing_cte._restates or cte is existing_cte:
                ...
                if (cte._is_clone_of is not None or existing_cte._is_clone_of is not None) and cte.compare(
                    existing_cte
                ):
                    is_new_cte = False
                else:
        ...
        if is_new_cte:
            ...
            if not cte_pre_alias_name and cte not in self_ctes:
                ...
                if cte.recursive:
                    ...
                    text += "(%s)" % (
                        ", ".join(
                            self.preparer.format_label_name(ident, anon_map=self.anon_map) for ident in recur_cols
                        )
                    )
    ...
    def visit_alias(
        ...
        **kwargs,
        ...
        if lateral:
            ...
            if from_linter and "lateral_from_linter" not in kwargs and "enclosing_lateral" in kwargs:
        ...
        if ashint:
            ...
            ret = inner + self.get_render_as_alias_suffix(self.preparer.format_alias(alias, alias_name))
                ...
                ret += "(%s)" % (
                    ", ".join(
                        ...
                        % (
                            ...
                            (
                                " %s" % self.dialect.type_compiler_instance.process(col.type, **kwargs)
    ...
    def _label_returning_column(self, stmt, column, populate_result_map, column_clause_args=None, **kw):
    ...
    def _label_select_column(
        ...
        assert within_columns_clause, (
            "_label_select_column is only relevant within " "the columns clause of a SELECT or RETURNING"
        )
    ...
    **kw)`` and returns a select object.   this is used for structural changes
    ...
    def group_by_clause(self, select, **kw):
        ...
        group_by = self._generate_delimited_list(select._group_by_clauses, OPERATORS[operators.comma_op], **kw)
        if group_by:
    ...
    def order_by_clause(self, select, **kw):
        ...
        order_by = self._generate_delimited_list(select._order_by_clauses, OPERATORS[operators.comma_op], **kw)
        ...
        **kw: Any,
        ...
        columns = [
            ...
            for (
                ...
                column,
                repeated,
            ) in stmt._generate_columns_plus_names(True, cols=base._select_iterables(returning_cols))
        ]
        ...
        **kwargs,
        ...
        if asfrom or ashint:
            ...
            if use_schema and effective_schema:
                ret = self.preparer.quote_schema(effective_schema) + "." + self.preparer.quote(table.name)
            else:
                ret = self.preparer.quote(table.name)

                if not effective_schema and ambiguous_table_name_map and table.name in ambiguous_table_name_map:
                    anon_name = self._truncated_identifier("alias", ambiguous_table_name_map[table.name])

                    ret = ret + self.get_render_as_alias_suffix(self.preparer.format_alias(None, anon_name))
    ...
    def _setup_crud_hints(self, stmt, table_text):
        ...
        if stmt.table in dialect_hints:
            table_text = self.format_from_hint_text(table_text, stmt.table, dialect_hints[stmt.table], True)
    ...
    _sentinel_col_non_autoinc_lookup = util.immutabledict(
        {
            _SentinelDefaultCharacterization.CLIENTSIDE: (InsertmanyvaluesSentinelOpts._SUPPORTED_OR_NOT),
    ...
    def _deliver_insertmanyvalues_batches(
        ...
        if rst:
            insert_crud_params = [(col, key, rst(expr), st) for col, key, expr, st in insert_crud_params]
        ...
        if not self.positional:
            ...
            keys_to_replace = all_keys.intersection(
                escaped_bind_names.get(key, key) for _, _, _, bind_keys in insert_crud_params for key in bind_keys
            )
            ...
            if all_names_we_will_expand:
                ...
                all_expand_positions = {idx for idx, name in enumerate(positiontup) if name in all_names_we_will_expand}
            ...
            if self._numeric_binds:
                ...
                executemany_values_w_comma = re.sub(rf"{escaped}\d+", "%s", executemany_values_w_comma)
        ...
        while batches:
            ...
            if self.positional:
                ...
                if num_ins_params == len(batch[0]):
                    ...
                    batch_iterator = (b[expand_pos_lower_index:expand_pos_upper_index] for b in batch)
                ...
                if imv.embed_values_counter:
                    ...
                    expanded_values_string = (executemany_values_w_comma * current_batch_size)[:-2]
                ...
                if self._numeric_binds and num_ins_params > 0:
                    ...
                    positions = tuple(f"{self._numeric_binds_identifier_char}{i}" for i in range(start, end))
                ...
                for i, param in enumerate(batch):
                    ...
                    replaced_parameters.update({f"{key}__{i}": param[key] for key in keys_to_replace})
            ...
            yield _InsertManyValuesBatch(
                ...
                processed_setinputsizes,
                batch,
                ([_sentinel_from_params(cb) for cb in compiled_batch] if _sentinel_from_params else []),
                current_batch_size,
                batchnum,
                total_batches,
                sort_by_parameter_order,
                ...
                if named_sentinel_params is None:
                    ...
                    if self.dialect.insertmanyvalues_implicit_sentinel & InsertmanyvaluesSentinelOpts.ANY_AUTOINCREMENT:
                        ...
                        assert not add_sentinel_cols[0]._insert_sentinel, (
                            "sentinel selection rules should have prevented " "us from getting here for this dialect"
                        )
    ...
    def update_from_clause(self, update_stmt, from_table, extra_froms, from_hints, **kw):
        ...
        """
        raise NotImplementedError("This backend does not support multiple-table " "criteria within UPDATE")
    ...
    def visit_update(self, update_stmt, visiting_cte=None, **kw):
        ...
        text += " SET "
        text += ", ".join(
            expr + "=" + value for _, expr, value, _ in cast("List[Tuple[Any, str, str, Any]]", crud_params)
        )
    ...
    def delete_extra_from_clause(self, update_stmt, from_table, extra_froms, from_hints, **kw):
        ...
        """
        raise NotImplementedError("This backend does not support multiple-table " "criteria within DELETE")
    ...
    def visit_create_table(self, create, **kw):
        ...
        for create_column in create.columns:
            ...
            try:
                ...
                raise exc.CompileError(
                    "(in table '%s', column '%s'): %s" % (table.description, column.name, ce.args[0])
                ) from ce
...
class GenericTypeCompiler(TypeCompiler):
    ...
    def visit_null(self, type_, **kw):
        raise exc.CompileError(
            "Can't generate DDL for %r; " "did you forget to specify a " "type on this Column?" % type_
        )
        ...
        def symbol_getter(obj):
            ...
            if obj._use_schema_map and (name is not None or includes_none):
                if name is not None and ("[" in name or "]" in name):
                    raise exc.CompileError(
                        "Square bracket characters ([]) not supported " "in schema translate name '%s'" % name
                    )
        ...
        def replace(m):
            ...
            if not effective_schema:
                ...
                if not effective_schema:
                    ...
                    raise exc.CompileError(
                        "Dialect has no default schema name; can't " "use None as dynamic schema target."
                    )
