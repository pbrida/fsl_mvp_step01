...
class CompoundSelect(HasCompileState, GenerativeSelect, ExecutableReturnsRows):
    ...
    def __init__(
        ...
        *selects: _SelectStatementForCompoundArgument,
        ...
        self.selects = [
            coercions.expect(roles.CompoundElementRole, s, apply_propagate_attrs=self).self_group(against=self)
            for s in selects
        ]
...
class SelectState(util.MemoizedSlots, CompileState):
    ...
    def get_columns_clause_froms(cls, statement: Select[Any]) -> List[FromClause]:
        return cls._normalize_froms(
            itertools.chain.from_iterable(element._from_objects for element in statement._raw_columns)
        )
    ...
    def _get_froms(self, statement: Select[Any]) -> List[FromClause]:
        ...
        return self._normalize_froms(
            itertools.chain(
                self.from_clauses,
                itertools.chain.from_iterable([element._from_objects for element in statement._raw_columns]),
                itertools.chain.from_iterable([element._from_objects for element in statement._where_criteria]),
            ),
    ...
    def _memoized_attr__label_resolve_dict(
        ...
        with_cols: Dict[str, ColumnElement[Any]] = {
            c._tq_label or c.key: c for c in self.statement._all_selected_columns if c._allow_label_resolve
        }
...
class Select(
    ...
    def __init__(self, *entities: _ColumnsClauseArgument[Any]):
        ...
        """
        self._raw_columns = [
            coercions.expect(roles.ColumnsClauseRole, ent, apply_propagate_attrs=self) for ent in entities
        ]
    ...
    def _copy_internals(self, clone: _CloneCallableType = _clone, **kw: Any) -> None:
        ...
        add_froms = {f for f in new_froms.values() if isinstance(f, Join)}.difference(all_the_froms).difference(
            existing_from_obj
        )
    ...
    def add_columns(self, *entities: _ColumnsClauseArgument[Any]) -> Select[Any]:
        ...
        self._raw_columns = self._raw_columns + [
            coercions.expect(roles.ColumnsClauseRole, column, apply_propagate_attrs=self) for column in entities
        ]
        return self

    def _set_entities(self, entities: Iterable[_ColumnsClauseArgument[Any]]) -> None:
        self._raw_columns = [
            coercions.expect(roles.ColumnsClauseRole, ent, apply_propagate_attrs=self) for ent in util.to_list(entities)
        ]
    ...
    def with_only_columns(
        ...
        **__kw: Any,
        ...
        self._raw_columns = [
            ...
            for c in coercions._expression_collection_was_a_list("entities", "Select.with_only_columns", entities)
        ]
        return self
    ...
    def select_from(self, *froms: _FromClauseArgument) -> Self:
        ...
        self._from_obj += tuple(
            coercions.expect(roles.FromClauseRole, fromclause, apply_propagate_attrs=self) for fromclause in froms
        )
        return self
    ...
    def correlate_except(
        ...
        *fromclauses: Union[Literal[None, False], _FromClauseArgument],
        ...
        if not fromclauses or fromclauses[0] in {None, False}:
            if len(fromclauses) > 1:
                raise exc.ArgumentError(
                    "additional FROM objects not accepted when " "passing None/False to correlate_except()"
                )
...
class ScalarSelect(roles.InElementRole, Generative, GroupedElement, ColumnElement[_T]):
    ...
    def columns(self) -> NoReturn:
        raise exc.InvalidRequestError(
            "Scalar Select expression has no " "columns; use this object directly " "within a column-level expression."
        )
        ...
        *,
        ...
        if proxy_compound_columns:
            fromclause._columns._populate_separate_keys(
                ...
                for c, extra_cols in zip(self.column_args, proxy_compound_columns, strict=False)
            )
        else:
