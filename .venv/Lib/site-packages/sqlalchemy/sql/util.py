...
def unwrap_order_by(clause: Any) -> Any:
    ...
    while stack:
        ...
        if isinstance(t, ColumnElement) and (
            not isinstance(t, UnaryExpression) or not operators.is_ordering_modifier(t.modifier)  # type: ignore
        ):
...
def reduce_columns(
    ...
    **kw: bool,
    ...
    cset_no_text: util.OrderedSet[ColumnElement[Any]] = column_set.difference(
        c
        for c in column_set
        if is_text_clause(c)  # type: ignore
    )
            ...
            if binary.operator == operators.eq:
                ...
                if binary.left in cols and binary.right in cols:
                    for c in reversed(cset_no_text):
                        if c.shares_lineage(binary.right) and (not only_synonyms or c.name == binary.left.name):
                            omit.add(c)
                            break
...
def criterion_as_pairs(
    ...
    if consider_as_foreign_keys and consider_as_referenced_keys:
        raise exc.ArgumentError(
            "Can only specify one of " "'consider_as_foreign_keys' or " "'consider_as_referenced_keys'"
        )
