...
from .elements import Slice, quoted_name
...
if TYPE_CHECKING:
    ...
    from .type_api import _BindProcessorType, _ComparatorFactory, _MatchedOnType, _ResultProcessorType
...
class SchemaType(SchemaEventTarget, TypeEngineMixin):
    ...
    def _set_table(self, column, table):
        ...
        event.listen(
            table,
            "before_create",
            util.portable_instancemethod(self._on_table_create, {"variant_mapping": variant_mapping}),
        )
        event.listen(
            table,
            "after_drop",
    ...
    def _is_impl_for_variant(self, dialect, kw):
        ...
        def _we_are_the_impl(typ):
            return (
                typ is self or isinstance(typ, ARRAY) and typ.item_type is self  # type: ignore[comparison-overlap]
            )
...
class Enum(String, SchemaType, Emulated, TypeEngine[Union[str, enum.Enum]]):
    ...
    def as_generic(self, allow_nulltype=False):
        ...
        return util.constructor_copy(self, self._generic_type_affinity, *args, _disable_warnings=True)
...
class Boolean(SchemaType, Emulated, TypeEngine[bool]):
    ...
    def _should_create_constraint(self, compiler, **kw):
        if not self._is_impl_for_variant(compiler.dialect, kw):
            return False
        return not compiler.dialect.supports_native_boolean and compiler.dialect.non_native_boolean_check_constraint
        ...
        def _setup_getitem(self, index):
            if not isinstance(index, str) and isinstance(index, collections_abc.Sequence):
                ...
                index = coercions.expect(
                    ...
                    bindparam_type=(JSON.JSONIntIndexType if isinstance(index, int) else JSON.JSONStrIndexType),
                )
                operator = operators.json_getitem_op
...
class TupleType(TypeEngine[Tuple[Any, ...]]):
    ...
    def __init__(self, *types: _TypeEngineArgument[Any]):
        ...
        self.types = [item_type() if isinstance(item_type, type) else item_type for item_type in types]
    ...
    def coerce_compared_value(self, op: Optional[OperatorType], value: Any) -> TypeEngine[Any]:
        if value is type_api._NO_VALUE_IN_LIST:
            ...
            return TupleType(
                *[typ.coerce_compared_value(op, elem) for typ, elem in zip(self.types, value, strict=False)]
            )
    ...
    def result_processor(self, dialect, coltype):
        raise NotImplementedError("The tuple type does not support being fetched " "as a column in a result row.")
