...
def _get_crud_params(
    ...
    **kw: Any,
    ...
    if parameters and stmt_parameter_tuples:
        check = (
            set(parameters).intersection(_column_as_key(k) for k, v in stmt_parameter_tuples).difference(check_columns)
        )
        if check:
...
def _scan_cols(
    ...
    if compile_state._parameter_ordering:
        ...
        cols = [stmt.table.c[key] for key in parameter_ordering if isinstance(key, str) and key in stmt.table.c] + [
...
def _append_param_parameter(
    ...
    if coercions._is_literal(value):
        ...
        value = _create_bind_param(
            ...
            name=(
                _col_bind_name(c)
                if not _compile_state_isinsert(compile_state) or not compile_state._has_multi_parameters
    ...
    if (
        ...
        and (
            ...
            and (
                ...
                (c.default is not None and c.default.is_sequence and compiler.dialect.supports_sequences)
                or
        ...
        c.default is None and c.server_default is None and not c.nullable and c is not stmt.table._autoincrement_column
    ):
        # no .default, no .server_default, not autoincrement, we have
        # no indication this primary key column will have any value
        _warn_pk_with_no_anticipated_value(c)
                ...
                if c.onupdate.is_clause_element:
                    ...
                    values.append(
                        (
                            c,
                            compiler.process(c, include_table=include_table),
                            _create_update_prefetch_bind_param(compiler, c, name=_col_bind_name(c), **kw),
                            (c.key,),
                        )
                    )
            elif c.server_onupdate is not None:
...
def _get_returning_modifiers(compiler, stmt, compile_state, toplevel):
    ...
    need_pks = (
        ...
        and (not compiler.for_executemany or (dialect.insert_executemany_returning and stmt._return_defaults))
...
def _warn_pk_with_no_anticipated_value(c):
    msg = (
        ...
        "Primary key columns typically may not store NULL." % (c.table.fullname, c.name, c.table.fullname)
    )
