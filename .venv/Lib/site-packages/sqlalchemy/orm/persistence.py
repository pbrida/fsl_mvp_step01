...
from . import attributes, loading, sync
    ...
    for table, mapper in base_mapper._sorted_tables.items():
        ...
        update = (
            (
                ...
                connection,
                (
                    mapper._get_committed_state_attr_by_column(state, state_dict, mapper.version_id_col)
    ...
    *,
    ...
    for state, state_dict, mapper, connection in states_to_insert:
        ...
        if not bulk:
            ...
            for colkey in (
                mapper._insert_cols_as_none[table].difference(params).difference([c.key for c in value_params])
            ):
                params[colkey] = None
    ...
    for (
        ...
        if bulk:
            ...
            pk_params = {
                ...
                for propkey in set(propkey_to_col).intersection(mapper._pk_attr_keys_by_table[table])
            }
            ...
            for col in pks:
                ...
                if pk_params[col._label] is None:
                    raise orm_exc.FlushError(
                        "Can't update table %s using NULL for primary " "key value on column %s" % (table, col)
                    )
...
def _collect_delete_commands(base_mapper, uowtransaction, table, states_to_delete):
    ...
    for (
        ...
        for col in mapper._pks_by_table[table]:
            ...
            if value is None:
                raise orm_exc.FlushError(
                    "Can't delete from table %s " "using NULL for primary " "key value on column %s" % (table, col)
                )
    ...
    for (
        ...
        if (
            ...
            if returning_is_required_anyway or (table.implicit_returning and not hasvalue and len(records) > 1):
                ...
                ) or (not deterministic_results_reqd and connection.dialect.insert_executemany_returning):
                    do_executemany = True
            ...
            if do_executemany:
                ...
                for (
                    ...
                    for pk, col in zip(primary_key, mapper._pks_by_table[table], strict=False):
                        prop = mapper_rec._columntoproperty[col]
        ...
        if (
            ...
            if only_warn:
                util.warn(
                    ...
                    "configuration to prevent this warning." % (table.description, expected, rows_matched)
                )
            else:
                raise orm_exc.StaleDataError(
                    "DELETE statement on table '%s' expected to "
...
def _sort_states(mapper, states):
    ...
    try:
        ...
        raise sa_exc.InvalidRequestError(
            "Could not sort objects by primary key; primary key " "values must be sortable in Python (was: %s)" % err
        ) from err
