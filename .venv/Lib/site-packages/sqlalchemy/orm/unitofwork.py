        ...
        if hashkey in self.attributes:
            ...
            if not cached_passive & attributes.SQL_OK and passive & attributes.SQL_OK:
                ...
                history = impl.get_history(
                    state,
                    state.dict,
                    attributes.PASSIVE_OFF | attributes.LOAD_AGAINST_COMMITTED | attributes.NO_RAISE,
                )
        ...
        if not self.session._contains_state(state):
            ...
            if not state.deleted and operation is not None:
                util.warn(
                    ...
                    "along '%s' will not proceed" % (orm_util.state_class_str(state), operation, prop)
                )
            return False
...
class ProcessState(PostSortRec):
    ...
    def execute_aggregate(self, uow, recs):
        ...
        our_recs = [
            r
            for r in recs
            if r.__class__ is cls_ and r.dependency_processor is dependency_processor and r.isdelete is isdelete
        ]
