...
from ..util.typing import Protocol, TypedDict, _AnnotationScanType, is_fwd_ref, is_literal, typing_get_args
from . import attributes, clsregistry, instrumentation, mapperlib
...
from .util import _extract_mapped_subtype, _is_mapped_annotation, class_mapper, de_stringify_annotation
...
class _MapperConfig:
    ...
    def setup_mapping(
        ...
        defer_map = _get_immediate_cls_attr(cls_, "_sa_decl_prepare_nocascade", strict=True) or hasattr(
            cls_, "_sa_decl_prepare"
        )
            ...
            def local_attributes_for_class() -> Iterable[Tuple[str, Any, Any, bool]]:
                for name in names:
                    ...
                    if field and sa_dataclass_metadata_key in field.metadata:
                        yield (
                            field.name,
                            _as_dc_declaredattr(field.metadata, fixed_sa_dataclass_metadata_key),
        ...
        for name, obj, annotation, is_dataclass in attributes_for_class():
            if not fixed_table and obj is None and _is_mapped_annotation(annotation, cls, originating_class):
                ...
                if name not in dict_ and not (
                    "__table__" in dict_ and (getattr(obj, "name", None) or name) in dict_["__table__"].c
                ):
                    if obj.foreign_keys:
        ...
        for k in list(collected_attributes):
            ...
            # to change that now (2.0 betas)
                ...
                _undefer_column_name(
                    k,
                    self.column_copies.get(value, value),  # type: ignore
                )
            else:
                if isinstance(value, _IntrospectsAnnotations):
                    (
                        ...
                        originating_module,
                        originating_class,
                    ) = self.collected_annotations.get(k, (None, None, None, False, None, None, None))
                ...
                if isinstance(value, _DCAttributeOptions):
                    if value._has_dataclass_arguments and not look_for_dataclass_things:
                        ...
                        args = {
                            a
                            for a in argnames
                            if getattr(value._attribute_options, f"dataclasses_{a}") is not _NoArg.NO_ARG
                        }
        ...
        for name, keys in name_to_prop_key.items():
            if len(keys) > 1:
                util.warn(
                    ...
                    "Consider using orm.synonym instead" % (self.classname, name, (", ".join(sorted(keys))))
                )
    ...
    def _prepare_mapper_arguments(self, mapper_kw: _MapperKwArgs) -> None:
        ...
        if "inherits" in mapper_args:
            ...
            if inherits_arg is not self.inherits:
                raise exc.InvalidRequestError(
                    "mapper inherits argument given for non-inheriting " "class %s" % (mapper_args["inherits"])
                )
