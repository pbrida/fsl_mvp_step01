...
import itertools
from typing import TYPE_CHECKING, Any, Dict, Iterable, List, Optional, Set, Tuple, Type, TypeVar, Union, cast

from .. import exc as sa_exc
from .. import future, inspect, sql, util
from ..sql import coercions, expression, roles, visitors
from ..sql import util as sql_util
from ..sql._typing import _TP, is_dml, is_insert_update, is_select_base
from ..sql.base import CacheableOptions, CompileState, Executable, Generative, Options, _select_iterables
    ...
    def create_for_statement(
        ...
        **kw: Any,
        ...
        self._label_convention = self._column_naming_convention(
            (statement._label_style if not statement._is_textual and not statement.is_dml else LABEL_STYLE_NONE),
            self.use_legacy_query_style,
        )
...
class ORMSelectCompileState(ORMCompileState, SelectState):
    ...
    def _setup_for_generate(self):
        ...
        if query._having_criteria:
            self._having_criteria = tuple(
                current_adapter(crit, True) if current_adapter else crit for crit in query._having_criteria
            )
        ...
        if query._correlate:
            ...
            self.correlate = tuple(
                util.flatten_iterator(
                    sql_util.surface_selectables(s) if s is not None else None for s in query._correlate
                )
            )
        elif query._correlate_except is not None:
            self.correlate_except = tuple(
                util.flatten_iterator(
                    sql_util.surface_selectables(s) if s is not None else None for s in query._correlate_except
                )
            )
        ...
        if hasattr(info, "mapper"):
            if not info.is_aliased_class:
                raise sa_exc.ArgumentError(
                    "A selectable (FromClause) instance is " "expected when the base alias is being set."
                )
            else:
    ...
    def _compound_eager_statement(self):
        ...
        if self.order_by:
            ...
            unwrapped_order_by = [
                (elem.element if isinstance(elem, sql.elements._label_reference) else elem) for elem in self.order_by
            ]
    ...
    def _join_check_and_adapt_right_side(self, left, right, onclause, prop):
        ...
        if right_mapper and (
            right_mapper.with_polymorphic or isinstance(right_mapper.persist_selectable, expression.Join)
        ):
            for from_obj in self.from_clauses or [l_info.selectable]:
                if sql_util.selectables_overlap(l_info.selectable, from_obj) and sql_util.selectables_overlap(
...
class _BundleEntity(_QueryEntity):
    ...
    def row_processor(self, context, result):
        procs, labels, extra = zip(*[ent.row_processor(context, result) for ent in self._entities], strict=False)
