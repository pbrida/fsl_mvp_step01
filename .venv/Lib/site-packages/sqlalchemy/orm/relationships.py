...
from .. import Exists, log, schema, sql, util
...
class _RelationshipArg(Generic[_T1, _T2]):
    ...
    def _resolve_against_registry(self, clsregistry_resolver: Callable[[str, bool], _class_resolver]) -> None:
...
class RelationshipProperty(_IntrospectsAnnotations, StrategizedProperty[_T], log.Identified):
    ...
    class Comparator(util.MemoizedSlots, PropComparator[_PT]):
        ...
        def and_(self, *criteria: _ColumnExpressionArgument[bool]) -> PropComparator[Any]:
            ...
            """
            exprs = tuple(
                coercions.expect(roles.WhereHavingRole, clause) for clause in util.coerce_generator_arg(criteria)
            )
            ...
            if self.prop.direction == MANYTOONE:
                ...
                def state_bindparam(
                    ...
                    return sql.bindparam(
                        ...
                        unique=True,
                        callable_=self.prop._get_attr_w_warn_on_none(self.prop.mapper, state, dict_, remote_col),
                    )
    ...
    def _get_attr_w_warn_on_none(
        ...
        def _go() -> Any:
            ...
            current_value = mapper._get_state_attr_by_column(
                ...
                column,
                passive=(
                    PassiveFlag.PASSIVE_OFF if state.persistent else PassiveFlag.PASSIVE_NO_FETCH ^ PassiveFlag.INIT_OK
                ),
            )
    ...
    def _check_sync_backref(rel_a: RelationshipProperty[Any], rel_b: RelationshipProperty[Any]) -> None:
        if rel_a.viewonly and rel_b.sync_backref:
            raise sa_exc.InvalidRequestError(
                "Relationship %s cannot specify sync_backref=True since %s " "includes viewonly=True." % (rel_b, rel_a)
            )
        ...
        if isinstance(resolved_argument, type):
            ...
            if not hasattr(entity, "mapper"):
                raise sa_exc.ArgumentError(
                    ...
                    "a class or a mapper argument (received: %s)" % (self.key, type(resolved_argument))
                )
    ...
    def _create_joins(
        ...
        aliased = aliased or (
            ...
            and (source_selectable is not self.parent._with_polymorphic_selectable or source_selectable._is_subquery)
        )
...
class JoinCondition:
    ...
    def _determine_joins(self) -> None:
        ...
        if self.secondaryjoin is not None and self.secondary is None:
            raise sa_exc.ArgumentError(
                "Property %s specified with secondary " "join condition but " "no secondary argument" % self.prop
            )
            ...
            if to_ not in self._track_overlapping_sync_targets:
                ...
                for pr, fr_ in prop_to_from.items():
                    if (
                        ...
                        and (self.prop.key != pr.key or not self.prop.parent.common_parent(pr.parent))
                    ):
                ...
                if other_props:
                    util.warn(
                        ...
                        % (
                            ...
                            from_,
                            to_,
                            ", ".join(sorted("'%s' (copies %s to %s)" % (pr, fr_, to_) for (pr, fr_) in other_props)),
    ...
    def join_targets(
        ...
        if aliased:
            if secondary is not None:
                ...
                secondary_aliasizer = ClauseAdapter(dest_selectable, equivalents=self.child_equivalents).chain(
                    primary_aliasizer
                )
    ...
    def create_lazy_clause(
        ...
        def col_to_bind(element: ColumnElement[Any], **kw: Any) -> Optional[BindParameter[Any]]:
            if (
                ...
                and ((has_secondary and element in lookup) or (not has_secondary and "remote" in element._annotations))
            ):
