...
from .. import event, inspection, log, schema, sql, util
from .. import exc as sa_exc
from ..event import EventTarget, dispatcher
from ..sql import TableClause, coercions, expression, operators, roles, visitors
...
from . import attributes, instrumentation, loading, properties
...
class Mapper(
    ...
    def __init__(
        ...
        polymorphic_on: Optional[Union[_ORMColumnExprArgument[Any], str, MapperProperty[Any]]] = None,
    ...
    _validate_polymorphic_identity: Optional[Callable[[Mapper[_O], InstanceState[_O], _InstanceDict], None]]
    ...
    def _configure_inheritance(self):
        ...
        if self.inherits:
            if not issubclass(self.class_, self.inherits.class_):
                raise sa_exc.ArgumentError(
                    "Class '%s' does not inherit from '%s'" % (self.class_.__name__, self.inherits.class_.__name__)
                )
    ...
    def _str_arg_to_mapped_col(self, argname: str, key: str) -> Column[Any]:
        try:
            ...
            raise sa_exc.ArgumentError(
                f"Can't determine {argname} column '{key}' - " "no attribute is mapped to this name."
            ) from err
        try:
            expr = prop.expression
        except AttributeError as ae:
            raise sa_exc.ArgumentError(
                f"Can't determine {argname} column '{key}'; " "property does not refer to a single mapped Column"
            ) from ae
        ...
        if self.inherits and not self.concrete and not self._primary_key_argument:
            ...
            if coerced_pk_arg:
                primary_key = [
                    ...
                    for cc, c in ((self.persist_selectable.corresponding_column(c), c) for c in coerced_pk_arg)
                ]
            else:
        ...
        if self.polymorphic_on is not None:
            ...
            if self.polymorphic_on in self._columntoproperty:
                ...
                if not isinstance(self.polymorphic_on, properties.ColumnProperty):
                    raise sa_exc.ArgumentError(
                        "Only direct column-mapped " "property or SQL expression " "can be passed for polymorphic_on"
                    )
    ...
    def _reconcile_prop_with_incoming_columns(
        ...
        if (
            (not self._inherits_equated_pairs or (equated_pair_key not in self._inherits_equated_pairs))
    ...
    def __str__(self) -> str:
        return "Mapper[%s%s(%s)]" % (
            self.class_.__name__,
            self.non_primary and " (non-primary)" or "",
            (self.local_table.description if self.local_table is not None else self.persist_selectable.description),
        )
    ...
    def _is_orphan(self, state: InstanceState[_O]) -> bool:
        ...
        for mapper in self.iterate_to_root():
            for key, cls in mapper._delete_orphans:
                ...
                has_parent = attributes.manager_of_class(cls).has_parent(state, key, optimistic=state.has_identity)
    ...
    def _selectable_from_mappers(self, mappers: Iterable[Mapper[Any]], innerjoin: bool) -> FromClause:
        ...
        for m in mappers:
            ...
            if m.concrete:
                raise sa_exc.InvalidRequestError(
                    "'with_polymorphic()' requires 'selectable' argument " "when concrete-inheriting mappers are used."
                )
            elif not m.single:
                if innerjoin:
    ...
    def _version_id_has_server_side_value(self) -> bool:
        ...
        if vid_col is None:
            ...
            return vid_col.server_default is not None or (
                vid_col.default is not None and (not vid_col.default.is_scalar and not vid_col.default.is_callable)
            )
    ...
    def _single_table_criterion(self):
        if self.single and self.inherits and self.polymorphic_on is not None:
            return self.polymorphic_on._annotate({"parententity": self, "parentmapper": self}).in_(
                [m.polymorphic_identity for m in self.self_and_descendants if not m.polymorphic_abstract]
            )
        else:
            return None
    ...
    def _pk_keys_by_table(self):
        return {table: frozenset([col.key for col in pks]) for table, pks in self._pks_by_table.items()}
        ...
        return {
            table: frozenset(
                [
                    ...
                    if col.server_default is not None or (col.default is not None and col.default.is_clause_element)
                ]
            )
            for table, columns in self._cols_by_table.items()
        }
        ...
        return {
            table: frozenset(
                [
                    col
                    for col in cast("Iterable[Column[Any]]", columns)
                    if col.server_onupdate is not None or (col.onupdate is not None and col.onupdate.is_clause_element)
                ]
            )
            for table, columns in self._cols_by_table.items()
        }
    ...
    def _columns_plus_keys(self, polymorphic_mappers=()):
        ...
        return [(prop.key, prop.columns[0]) for prop in poly_properties if isinstance(prop, properties.ColumnProperty)]
    ...
    def _filter_properties(self, type_: Type[_MP]) -> util.ReadOnlyProperties[_MP]:
        self._check_configure()
        return util.ReadOnlyProperties(util.OrderedDict((k, v) for k, v in self._props.items() if isinstance(v, type_)))
    ...
    def _should_exclude(self, name, assigned_name, local, column):
        ...
        if local:
            if self.class_.__dict__.get(assigned_name, None) is not None and self._is_userland_descriptor(
        ...
        if self.exclude_properties is not None and (
            name in self.exclude_properties or (column is not None and column in self.exclude_properties)
        ):
    ...
    def _identity_key_from_state(
        ...
        return (
            self._identity_class,
            tuple([manager[prop.key].impl.get(state, dict_, passive) for prop in self._identity_key_props]),
    ...
    def _get_committed_attr_by_column(self, obj, column):
        ...
        return self._get_committed_state_attr_by_column(state, dict_, column, passive=PassiveFlag.PASSIVE_OFF)

    def _get_committed_state_attr_by_column(self, state, dict_, column, passive=PassiveFlag.PASSIVE_RETURN_NO_VALUE):
        prop = self._columntoproperty[column]
...
def _dispose_registries(registries: Set[_RegistryType], cascade: bool) -> None:
    ...
    for reg in registry._recurse_with_dependents(registries):
        if not cascade and reg._dependents.difference(orig):
            raise sa_exc.InvalidRequestError(
                "Registry has dependent registries that are not disposed; " "pass cascade=True to clear these also"
            )
...
class _ColumnMapping(Dict["ColumnElement[Any]", "MapperProperty[Any]"]):
    ...
    def __missing__(self, column):
        ...
        raise orm_exc.UnmappedColumnError("No column %s is configured on mapper %s..." % (column, self.mapper))
