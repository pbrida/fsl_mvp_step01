...
from typing import Any, Callable, Iterator, NoReturn, Optional, Tuple, TypeVar, Union, cast
        ...
        def _go(fn: _F, self: Any, *arg: Any, **kw: Any) -> Any:
            ...
            try:
                ...
                if self._state is current_state:
                    raise sa_exc.IllegalStateChangeError(
                        f"Method '{fn.__name__}()' failed to " "change state " f"to {moves_to!r} as expected",
                        code="isce",
                    )
                elif existing_fn:
                    raise sa_exc.IllegalStateChangeError(
                    ...
                    raise sa_exc.IllegalStateChangeError(
                        f"Method '{fn.__name__}()' caused an unexpected " f"state change to {self._state!r}",
                        code="isce",
                    )
