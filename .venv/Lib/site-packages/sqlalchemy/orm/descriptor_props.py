...
from typing import TYPE_CHECKING, Any, Callable, Dict, List, NoReturn, Optional, Sequence, Tuple, Type, TypeVar, Union
...
from .interfaces import MapperProperty, PropComparator, _AttributeOptions, _IntrospectsAnnotations, _MapsColumns
...
if typing.TYPE_CHECKING:
    ...
    from ..util.typing import CallableReference, DescriptorReference, RODescriptorReference, _AnnotationScanType
...
class CompositeProperty(_MapsColumns[_CC], _IntrospectsAnnotations, DescriptorProperty[_CC]):
        ...
        if is_dataclass(self.composite_class):
            ...
            for attr in self.attrs:
                if isinstance(attr, (MappedColumn, schema.Column)) and attr.name is None:
                    raise sa_exc.ArgumentError(
                        "Composite class column arguments must be named " "unless a dataclass is used"
                    )
        self._init_accessor()
    ...
    def _populate_composite_bulk_save_mappings_fn(
        ...
        def populate(dest_dict: Dict[str, Any]) -> None:
            dest_dict.update({key: val for key, val in zip(attrs, get_values(dest_dict.pop(self.key)), strict=False)})
    ...
    class Comparator(PropComparator[_PT]):
        ...
        def _compare(self, operator: OperatorType, other: Any) -> ColumnElement[bool]:
            ...
            comparisons = [operator(a, b) for a, b in zip(self.prop._comparable_elements, values, strict=False)]
            if self._adapt_to_entity:
