...
class ClassManager(
    ...
    @util.deprecated(
        "1.4",
        message="The ClassManager.deferred_scalar_loader attribute is now " "named expired_attribute_loader",
    )
    ...
    def __init__(self, class_):
        ...
        self._bases = [
            ...
            for mgr in cast(
                ...
                [opt_manager_of_class(base) for base in self.class_.__bases__ if isinstance(base, type)],
            )
            if mgr is not None
        ]
    ...
    def install_descriptor(self, key: str, inst: QueryableAttribute[Any]) -> None:
        if key in (self.STATE_ATTR, self.MANAGER_ATTR):
            raise KeyError(
                "%r: requested attribute name conflicts with " "instrumentation attribute of the same name." % key
            )
