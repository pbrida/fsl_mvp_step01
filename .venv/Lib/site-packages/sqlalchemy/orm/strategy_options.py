...
import typing
from typing import Any, Callable, Dict, Iterable, Optional, Sequence, Tuple, Type, TypeVar, Union, cast, overload
...
from ._typing import insp_is_aliased_class, insp_is_attribute, insp_is_mapper, insp_is_mapper_property
...
class _AbstractLoad(traversals.GenerativeOnTraversal, LoaderOption):
    ...
    def process_compile_state(self, compile_state: ORMCompileState) -> None:
        ...
        self._process(
            ...
            not bool(compile_state.current_path) and not compile_state.compile_options._for_refresh_state,
        )
    ...
    def _chop_path(
        ...
        for i, (c_token, p_token) in enumerate(zip(to_chop, path.natural_path, strict=False)):
            if isinstance(c_token, str):
                if i == 0 and (c_token.endswith(f":{_DEFAULT_TOKEN}") or c_token.endswith(f":{_WILDCARD_TOKEN}")):
                    return to_chop
                elif (
                    c_token != f"{_RELATIONSHIP_TOKEN}:{_WILDCARD_TOKEN}" and c_token != p_token.key  # type: ignore
                ):
                    return None
...
class Load(_AbstractLoad):
    ...
    def _adjust_for_extra_criteria(self, context: QueryContext) -> Load:
        ...
        cloned.context = tuple(
            (process(value, replacement_cache_key, orig_cache_key) if value._extra_criteria else value)
            for value in self.context
        )
        return cloned
...
class _WildcardLoad(_AbstractLoad):
    ...
    def _find_entity_basestring(
        ...
        for ent in entities:
            ...
            if raiseerr:
                raise sa_exc.ArgumentError(
                    "Query has only expression-based entities - " f'can\'t find property named "{token}".'
                )
            else:
                return None
...
class _TokenStrategyLoad(_LoadElement):
    ...
    def _prepare_for_compile_state(
        ...
        return [
            ("loader", natural_path)
            for natural_path in (cast(TokenRegistry, effective_path)._generate_natural_for_superclasses())
        ]
...
def _raise_for_does_not_link(path, attrname, parent_entity):
    if len(path) > 1:
        ...
        raise sa_exc.ArgumentError(f'ORM mapped attribute "{attrname}" does not ' f'link mapped class "{path[-1]}"')
