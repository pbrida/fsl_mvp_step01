...
import weakref
from typing import TYPE_CHECKING, Any, Callable, Dict, Generic, Iterable, Optional, Set, Tuple, Union
...
class InstanceState(interfaces.InspectionAttrInfo, Generic[_O]):
    ...
    def __setstate__(self, state_dict: Dict[str, Any]) -> None:
        ...
        self.__dict__.update([(k, state_dict[k]) for k in ("key", "load_options") if k in state_dict])
        if self.key:
        ...
        if attr:
            ...
            if is_userland and attr.key not in dict_:
                raise sa_exc.InvalidRequestError(
                    "Can't flag attribute '%s' modified; it's not present in " "the object state" % attr.key
                )
