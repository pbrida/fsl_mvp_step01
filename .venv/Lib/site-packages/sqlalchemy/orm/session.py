...
from . import attributes, bulk_persistence, context, descriptor_props, exc, identity, loading, query
...
if typing.TYPE_CHECKING:
    ...
    from ..sql._typing import _T0, _T1, _T2, _T3, _T4, _T5, _T6, _T7, _ColumnsClauseArgument, _InfoType
...
class ORMExecuteState(util.MemoizedSlots):
    ...
    def load_options(
        ...
        if not self.is_select:
            raise sa_exc.InvalidRequestError(
                "This ORM execution is not against a SELECT statement " "so there are no load options."
            )

        lo: Union[
            context.QueryContext.default_load_options,
            Type[context.QueryContext.default_load_options],
        ] = self.execution_options.get("_sa_orm_load_options", context.QueryContext.default_load_options)
        return lo
    ...
    def update_delete_options(
        ...
        if not self._is_crud:
            raise sa_exc.InvalidRequestError(
                "This ORM execution is not against an UPDATE or DELETE " "statement so there are no update options."
            )
        uo: Union[
            bulk_persistence.BulkUDCompileState.default_update_options,
            Type[bulk_persistence.BulkUDCompileState.default_update_options],
...
class SessionTransaction(_StateChange, TransactionalContext):
    ...
    def __init__(
        ...
        if session._close_state is _SessionCloseState.CLOSED:
            raise sa_exc.InvalidRequestError(
                "This Session has been permanently closed and is unable " "to handle any more transaction requests."
            )

        if nested:
            if not parent:
                raise sa_exc.InvalidRequestError(
                    "Can't start a SAVEPOINT transaction when no existing " "transaction is in progress"
                )
    ...
    def _begin(self, nested: bool = False) -> SessionTransaction:
        return SessionTransaction(
            self.session,
            (SessionTransactionOrigin.BEGIN_NESTED if nested else SessionTransactionOrigin.SUBTRANSACTION),
            self,
        )
    ...
    def rollback(self, _capture_exception: bool = False, _to_root: bool = False) -> None:
        ...
        if not rollback_err and not sess._is_clean():
            ...
            util.warn(
                "Session's state has been changed on " "a non-active transaction - this state " "will be discarded."
            )
...
class Session(_SessionClassMethods, EventTarget):
    ...
    def __init__(
        ...
        *,
        ...
        if not future:
            raise sa_exc.ArgumentError("The 'future' parameter passed to " "Session() may only be set to True.")
        ...
        if (
            join_transaction_mode and join_transaction_mode not in JoinTransactionMode.__args__  # type: ignore
        ):
            raise sa_exc.ArgumentError(f"invalid selection for join_transaction_mode: " f'"{join_transaction_mode}"')
    ...
    def refresh(
        ...
        if (
            ...
            raise sa_exc.InvalidRequestError("Could not refresh instance '%s'" % instance_str(instance))
    ...
    def _conditional_expire(self, state: InstanceState[Any], autoflush: Optional[bool] = None) -> None:
    ...
    def _expunge_states(self, states: Iterable[InstanceState[Any]], to_transient: bool = False) -> None:
        for state in states:
    ...
    def _get_impl(
        ...
        *,
        ...
        if is_dict:
            ...
            try:
                primary_key_identity = list(primary_key_identity[prop.key] for prop in mapper._identity_key_props)
        ...
        *,
        ...
        if key is None:
            ...
            key_is_persistent = LoaderCallableStatus.NEVER_SET not in key[1] and (
                not _none_set.intersection(key[1]) or (mapper.allow_partial_pks and not _none_set.issuperset(key[1]))
            )
        else:
    ...
    def _validate_persistent(self, state: InstanceState[Any]) -> None:
        if not self.identity_map.contains_state(state):
            raise sa_exc.InvalidRequestError("Instance '%s' is not persistent within this Session" % state_str(state))

    def _save_impl(self, state: InstanceState[Any]) -> None:
        if state.key is not None:
            raise sa_exc.InvalidRequestError(
                "Object '%s' already has an identity - " "it can't be registered as pending" % state_str(state)
            )
    ...
    def is_modified(self, instance: object, include_collections: bool = True) -> bool:
        ...
        * Scalar attributes may not have recorded the previously set
        ...
        for attr in state.manager.attributes:
            ...
            (added, unchanged, deleted) = attr.impl.get_history(state, dict_, passive=PassiveFlag.NO_CHANGE)
    ...
    def dirty(self) -> IdentitySet:
        ...
        """
        return IdentitySet([state.obj() for state in self._dirty_states if state not in self._deleted])
