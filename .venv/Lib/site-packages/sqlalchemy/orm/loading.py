...
from typing import TYPE_CHECKING, Any, Dict, Iterable, List, Mapping, Optional, Sequence, Tuple, TypeVar, Union
...
from . import attributes, path_registry
...
def instances(cursor: CursorResult[Any], context: QueryContext) -> Result[Any]:
    ...
    try:
        (process, labels, extra) = list(
            zip(
                *[query_entity.row_processor(context, cursor) for query_entity in context.compile_state._entities],
                strict=False,
            )
        )

        if context.yield_per and (context.loaders_require_buffering or context.loaders_require_uniquing):
    ...
    def _no_unique(entry):
        raise sa_exc.InvalidRequestError("Can't use the ORM yield_per feature in conjunction with unique()")
    ...
    # on the result.
    result._attributes = result._attributes.union(dict(filtered=filtered, is_single_entity=single_entity))
    ...
    *,
    ...
    if primary_key_identity is not None:
        ...
        params = {
            ...
            for id_val, primary_key in zip(primary_key_identity, mapper.primary_key, strict=False)
        }
    else:
        params = None
...
def _load_subclass_via_in(context, path, entity, polymorphic_from, option_entities):
    ...
    def do_load(context, path, states, load_only, effective_entity):
        ...
        context.session.execute(
            q2,
            dict(primary_keys=[state.key[1][0] if zero_idx else state.key[1] for state, load_attrs in states]),
        ).unique().scalars().all()
...
def load_scalar_attributes(mapper, state, attribute_names, passive):
    ...
    if not session:
        raise orm_exc.DetachedInstanceError(
            "Instance %s is not bound to a Session; " "attribute refresh operation cannot proceed" % (state_str(state))
        )
