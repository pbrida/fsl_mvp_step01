...
from typing import TYPE_CHECKING, Any, Dict, Iterable, Optional, TypeVar, Union, cast, overload
...
from . import attributes, context, evaluator, loading, persistence
from . import exc as orm_exc
from .base import NO_VALUE
from .context import AbstractORMCompileState, FromStatement, ORMFromStatementCompileState, QueryContext
...
class ORMDMLState(AbstractORMCompileState):
    ...
    def _get_multi_crud_kv_pairs(cls, statement, kv_iterator):
        ...
        return [
            dict(cls._get_orm_crud_kv_pairs(plugin_subject.mapper, statement, value_dict.items(), False))
...
class BulkUDCompileState(ORMDMLState):
        ...
        if sync is not None:
            if sync not in ("auto", "evaluate", "fetch", False):
                raise sa_exc.ArgumentError(
                    "Valid strategies for session synchronization " "are 'auto', 'evaluate', 'fetch', False"
                )
    ...
    def _get_matched_objects_on_criteria(cls, update_options, states):
        ...
        raw_data = [
            (state.obj(), state, state.dict) for state in states if state.mapper.isa(mapper) and not state.expired
        ]
    ...
    def _setup_for_bulk_insert(self, compiler):
        ...
        if self._dict_parameters:
            self._dict_parameters = {
                col: val for col, val in self._dict_parameters.items() if col.table is emit_insert_table
            }
...
class BulkORMUpdate(BulkUDCompileState, UpdateDMLState):
    ...
    def _setup_for_bulk_update(self, statement, compiler, **kw):
        ...
        if self._dict_parameters:
            self._dict_parameters = {
                col: val for col, val in self._dict_parameters.items() if col.table is emit_update_table
            }
        self.statement = statement
    ...
    def _do_post_synchronize_fetch(cls, session, statement, result, update_options):
        ...
        objs = [
            ...
            for identity_key in [
                ...
                if update_options._identity_token is None or identity_token == update_options._identity_token
            ]
