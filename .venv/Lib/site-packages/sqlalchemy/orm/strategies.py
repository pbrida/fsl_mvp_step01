...
from .. import event, inspect, log, sql, util
from .. import exc as sa_exc
from ..sql import util as sql_util
from ..sql import visitors
from ..sql.selectable import LABEL_STYLE_TABLENAME_PLUS_COL, Select
from . import attributes, interfaces, loading, path_registry, properties, query, relationships, unitofwork
    ...
    def init_class_attribute(self, mapper):
        ...
        active_history = (
            ...
            or (
                ...
                and mapper._columntoproperty.get(mapper.version_id_col, None) is self.parent_property
            )
        )
...
class DeferredColumnLoader(LoaderStrategy):
    ...
    def create_row_processor(
        ...
        if (
            ...
            self.parent_property._get_strategy((("deferred", False), ("instrument", True))).create_row_processor(
                context,
                query_entity,
                path,
                loadopt,
    ...
    def setup_query(
        ...
        **kw,
        ...
        if (
            (compile_state.compile_options._render_for_subquery and self.parent_property._renders_in_subqueries)
            ...
            self.parent_property._get_strategy((("deferred", False), ("instrument", True))).setup_query(
                compile_state,
                query_entity,
                path,
                loadopt,
    ...
    def _load_for_state(self, state, passive):
        ...
        if session is None:
            raise orm_exc.DetachedInstanceError(
                ...
                "deferred load operation of attribute '%s' cannot proceed" % (orm_util.state_str(state), self.key)
            )
...
class LazyLoader(AbstractRelationshipLoader, util.MemoizedSlots, log.Identified):
    ...
    def _load_for_state(
        ...
        if not state.key and (
            (not self.parent_property.load_on_pending and not state._load_pending) or not state.session_id
        ):
            return LoaderCallableStatus.ATTR_EMPTY
        ...
        if (
            ...
            and (
                ...
                not use_get or passive & PassiveFlag.RELATED_OBJECT_OK
            )
        ):
            self._invoke_raise_load(state, passive, "raise")
        ...
        if not session:
            ...
            raise orm_exc.DetachedInstanceError(
                "Parent instance %s is not bound to a Session; "
                "lazy load operation of attribute '%s' cannot proceed" % (orm_util.state_str(state), self.key)
            )
    ...
    def _get_ident_for_use_get(self, session, state, passive):
        ...
        return [get_attr(state, dict_, self._equated_columns[pk], passive=passive) for pk in self.mapper.primary_key]
        ...
        if not self.is_class_level or (loadopt and loadopt._extra_criteria):
            ...
            set_lazy_callable = (InstanceState._instance_level_callable_processor)(
                ...
                LoadLazyAttribute(
                    ...
                    self,
                    loadopt,
                    (loadopt._generate_extra_criteria(context) if loadopt._extra_criteria else None),
                ),
                key,
            )
...
class SubqueryLoader(PostLoader):
    ...
    def _get_leftmost(
        ...
        if is_root:
            ...
            # to adapt
            new_subq_path = current_compile_state._entities[orig_query_entity_index].entity_zero._path_registry[
                leftmost_prop
            ]
    ...
    def _apply_joins(self, q, to_join, left_alias, parent_alias, effective_entity):
        ...
        if ltj == 1:
            ...
            middle = [
                (
                    (orm_util.AliasedClass(item[0]) if not inspect(item[0]).is_aliased_class else item[0].entity),
                    item[1],
                )
...
class JoinedLoader(AbstractRelationshipLoader):
    ...
    def setup_query(
        ...
        **kwargs,
        ...
        user_defined_adapter = (
            self._init_user_defined_eager_proc(loadopt, compile_state, compile_state.attributes) if loadopt else False
        )
    ...
    def _generate_row_adapter(
        ...
        if with_poly_entity:
            ...
            to_adapt = orm_util.AliasedClass(
                self.mapper,
                alias=(alt_selectable._anonymous_fromclause(flat=True) if alt_selectable is not None else None),
                flat=True,
    ...
    def _create_eager_join(
        ...
        if additional_entity_criteria:
            extra_join_criteria += tuple(
                ae._resolve_where_criteria(self.mapper) for ae in additional_entity_criteria if ae.propagate_to_loaders
            )
    ...
    def _create_eager_adapter(self, context, result, adapter, path, loadopt):
        ...
        user_defined_adapter = (
            self._init_user_defined_eager_proc(loadopt, compile_state, context.attributes) if loadopt else False
        )
...
class SelectInLoader(PostLoader, util.MemoizedSlots):
    ...
    def _init_for_omit_join(self):
        ...
        pk_to_fk.update(
            (equiv, pk_to_fk[k]) for k in list(pk_to_fk) for equiv in self.parent._equivalent_columns.get(k, ())
        )
    ...
    def _load_for_path(
        ...
        if orig_query is context.query:
            ...
            new_options = [
                ...
                for orig_opt, uncached_opt in zip(cached_options, uncached_options, strict=False)
            ]
        ...
        while our_keys:
            ...
            data = {
                ...
                for k, v in context.session.execute(
                    q,
                    params={"primary_keys": [key[0] if query_info.zero_idx else key for key in chunk]},
                    execution_options=execution_options,
                ).unique()
            }
