...
from ..util.typing import Literal, Protocol, is_origin_of_cls, typing_get_origin
...
from .base import DynamicMapped, Mapped, ORMDescriptor, WriteOnlyMapped, _MappedAnnotationBase, opt_manager_of_class
    ...
    * ``identity_key(class, row=row, identity_token=token)``
    ...
    if class_ is not None:
        ...
        if row is None:
            ...
            return mapper.identity_key_from_primary_key(tuple(util.to_list(ident)), identity_token=identity_token)
        else:
...
class AliasedClass(inspection.Inspectable["AliasedInsp[_O]"], ORMColumnsClauseRole[_O]):
    ...
    def __init__(
        ...
        self._aliased_insp = AliasedInsp(
            ...
            (with_polymorphic_discriminator if with_polymorphic_discriminator is not None else mapper.polymorphic_on),
            base_alias,
            use_mapper_path,
            adapt_on_names,
    ...
    * ``polymorphic_on`` - an alternate column or SQL expression which
    ...
    def __init__(
        ...
        self._adapter = ORMAdapter(
            ...
            adapt_from_selectables={m.selectable for m in self.with_polymorphic_mappers if not adapt_on_names},
    ...
    def _with_polymorphic_factory(
        ...
        if selectable not in (None, False) and flat:
            raise sa_exc.ArgumentError(
                "the 'flat' and 'selectable' arguments cannot be passed " "simultaneously to with_polymorphic()"
            )
    ...
    def __str__(self):
        if self._is_with_polymorphic:
            return "with_polymorphic(%s, [%s])" % (
                ...
                ", ".join(mp.class_.__name__ for mp in self.with_polymorphic_mappers if mp is not self.mapper),
            )
        else:
...
class Bundle(
    ...
    def __init__(self, name: str, *exprs: _ColumnExpressionArgument[Any], **kw: Any):
        ...
        coerced_exprs = [coercions.expect(roles.ColumnsClauseRole, expr, apply_propagate_attrs=self) for expr in exprs]
        self.exprs = coerced_exprs

        self.c = self.columns = ColumnCollection(
            (getattr(col, "key", col._label), col) for col in [e._annotations.get("bundle", e) for e in coerced_exprs]
        ).as_readonly()
    ...
    def entity(self) -> Optional[_InternalEntityType[Any]]:
        ie: Optional[_InternalEntityType[Any]] = self.exprs[0]._annotations.get("parententity", None)
        return ie
...
class _ORMJoin(expression.Join):
    ...
    def __init__(
        ...
        if (
            not prop and getattr(right_info, "mapper", None) and right_info.mapper.single  # type: ignore
        ):
...
def with_parent(
    ...
    if isinstance(prop, str):
        ...
        if mapper_property is None or not prop_is_relationship(mapper_property):
            raise sa_exc.ArgumentError(f"Expected relationship property for with_parent(), " f"got {mapper_property}")
