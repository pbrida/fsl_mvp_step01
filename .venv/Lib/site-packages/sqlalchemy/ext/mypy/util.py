...
from mypy.types import CallableType, Instance, NoneType, Type, TypeVarType, UnboundType, UnionType, get_proper_type
...
def unbound_to_instance(api: SemanticAnalyzerPluginInterface, typ: Type) -> Type:
    ...
    if node is not None and isinstance(node, SymbolTableNode) and isinstance(node.node, TypeInfo):
        ...
        return Instance(
            bound_type,
            [(unbound_to_instance(api, arg) if isinstance(arg, UnboundType) else arg) for arg in typ.args],
        )
    else:
        return typ
