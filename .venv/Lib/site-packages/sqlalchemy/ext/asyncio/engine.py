...
from .base import GeneratorStartableContext, ProxyComparable, StartableContext, asyncstartablecontext
...
class AsyncConnection(
    ...
    async def start(
        self,
        is_ctxmanager: bool = False,  # noqa: U100
    ) -> AsyncConnection:
    ...
    async def stream(
        ...
        *,
        ...
        result = await greenlet_spawn(
            ...
            statement,
            parameters,
            execution_options=util.EMPTY_DICT.merge_with(execution_options, {"stream_results": True}),
            _require_await=True,
        )
    ...
    async def stream_scalars(
        ...
        *,
        ...
        async with self.stream(statement, parameters, execution_options=execution_options) as result:
    ...
    async def start(self, is_ctxmanager: bool = False) -> AsyncTransaction:
        ...
        self.sync_transaction = self._assign_proxied(
            await greenlet_spawn(
                self.connection._proxied.begin_nested if self.nested else self.connection._proxied.begin
            )
        )
        if is_ctxmanager:
