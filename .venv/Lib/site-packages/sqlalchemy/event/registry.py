...
class _EventKey(Generic[_ET]):
    ...
    def listen(self, *args: Any, **kw: Any) -> None:
        ...
        if once or once_unless_exception:
            self.with_wrapper(util.only_once(self._listen_fn, retry_on_exception=once_unless_exception)).listen(
                *args, **kw
            )
        else:
    ...
    def remove(self) -> None:
        ...
        if key not in _key_to_collection:
            raise exc.InvalidRequestError(
                "No listeners found for event %s / %r / %s " % (self.target, self.identifier, self.fn)
            )
