...
class DefaultDialect(Dialect):
    ...
    @util.deprecated(
        "2.0",
        "full_returning is deprecated, please use insert_returning, " "update_returning, delete_returning",
    )
    ...
    def validate_identifier(self, ident):
        if len(ident) > self.max_identifier_length:
            raise exc.IdentifierError(
                "Identifier '%s' exceeds maximum length of %d characters" % (ident, self.max_identifier_length)
            )
    ...
    def _set_connection_characteristics(self, connection, characteristics):
        characteristic_values = [
            (name, self.connection_characteristics[name], value) for name, value in characteristics.items()
        ]
                ...
                if imv.num_sentinel_columns and not imv_batch.is_downgraded:
                    ...
                    if not _sentinel_proc_initialized:
                        if composite_sentinel:
                            ...
                            _scalar_sentinel_proc = (imv.sentinel_columns[0]).type._cached_result_processor(
                    ...
                    if composite_sentinel:
                        rows_by_sentinel = {
                            tuple(
                                ...
                                for val, proc in zip(row[-_nsc:], _composite_sentinel_proc, strict=False)
                            ): row
                            for row in rows
                        }
...
class DefaultExecutionContext(ExecutionContext):
    ...
    def _init_compiled(
        ...
        flattened_processors: Mapping[str, _BindProcessorType[Any]] = processors  # type: ignore[assignment]

        if compiled.literal_execute_params or compiled.post_compile_params:
            if self.executemany:
                raise exc.InvalidRequestError(
                    "'literal_execute' or 'expanding' parameters can't be " "used with executemany()"
                )
    ...
    def _use_server_side_cursor(self):
        ...
        if self.dialect.server_side_cursors:
            ...
            use_server_side = self.execution_options.get("stream_results", True) and (
                ...
                or (
                    (not self.compiled or isinstance(self.compiled.statement, expression.TextClause))
    ...
    def _setup_ins_pk_from_implicit_returning(self, result, rows):
        ...
        return [getter(row, param) for row, param in zip(rows, compiled_params, strict=False)]
