...
from .result import IteratorResult, MergedResult, Result, ResultMetaData, SimpleResultMetaData, tuplegetter
from .row import Row
...
if typing.TYPE_CHECKING:
    ...
    from .result import _KeyIndexType, _KeyMapRecType, _KeyMapType, _KeyType, _ProcessorsType, _TupleGetterType
...
class CursorResultMetaData(ResultMetaData):
    ...
    def _splice_horizontally(self, other: CursorResultMetaData) -> CursorResultMetaData:
        ...
        return self._make_new_metadata(
            ...
            keymap_by_result_column_idx={
                metadata_entry[MD_RESULT_MAP_INDEX]: metadata_entry for metadata_entry in keymap.values()
            },
        )
    ...
    def _adapt_to_context(self, context: ExecutionContext) -> ResultMetaData:
        ...
        if keymap_by_position is None:
            ...
            # initialize lazily
            keymap_by_position = self._keymap_by_result_column_idx = {
                metadata_entry[MD_RESULT_MAP_INDEX]: metadata_entry for metadata_entry in self._keymap.values()
            }
    ...
    def _merge_textual_cols_by_position(self, context, cursor_description, result_columns):
        ...
        if num_ctx_cols > len(cursor_description):
            util.warn(
                ...
                "smaller than number of columns requested (%d)" % (num_ctx_cols, len(cursor_description))
            )
        seen = set()
            ...
            if raiseerr:
                if self._unpickled and isinstance(key, elements.ColumnElement):
                    ...
                    raise exc.NoSuchColumnError(
                        "Could not locate column in row for column '%s'" % util.string_or_unprintable(key)
                    ) from err
            else:
                return None
