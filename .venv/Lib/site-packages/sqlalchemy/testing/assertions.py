...
def int_within_variance(expected, received, variance):
    ...
    assert abs(received - expected) < deviance, "Given int value %s is not within %d%% of expected value %s" % (
        received,
        variance * 100,
        expected,
    )
...
class AssertsCompiledSQL:
    def assert_compile(
        ...
        if check_param_order and getattr(c, "params", None):
            ...
            msg = "Order of parameters %s does not match the order " "in the statement %s. Statement %r" % (
                values,
                expected,
                stmt,
            )
...
class ComparesTables:
    ...
    def assert_types_base(self, c1, c2):
        assert c1.type._compare_type_affinity(c2.type), "On column %r, type '%s' doesn't correspond to type '%s'" % (
            c1.name,
            c1.type,
            c2.type,
        )
...
class AssertsExecutionResults:
    ...
    def assert_statement_count_multi_db(self, dbs, counts):
        recs = [(self.sql_execution_asserter(db), db, count) for (db, count) in zip(dbs, counts, strict=False)]
        asserters = []
    ...
    def compare_table_index_with_expected(self, table: schema.Table, expected: list, dialect_name: str):
        ...
        for exp in expected:
            ...
            if exprs:
                ...
                for idx_exp, expr, col in zip(idx.expressions, exprs, exp["column_names"], strict=False):
                    if col is None:
