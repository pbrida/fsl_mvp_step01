...
def pytest_configure(config: pytest.Config):
    ...
    if plugin_base.exclude_tags or plugin_base.include_tags:
        new_expr = " and ".join(list(plugin_base.include_tags) + [f"not {tag}" for tag in plugin_base.exclude_tags])
...
class XDistHooks:
    def pytest_configure_node(self, node):
        ...
        asyncio._maybe_async_provisioning(provision.create_follower_db, node.workerinput["follower_ident"])
...
def pytest_collection_modifyitems(session, config, items):
    ...
    items[:] = [
        item
        for item in items
        if item.getparent(pytest.Class) is not None and not item.getparent(pytest.Class).name.startswith("_")
    ]
...
def _parametrize_cls(module, cls):
    ...
    for full_param_set in itertools.product(*[params for argname, params in _sa_parametrize]):
        ...
        for argname, param in zip([_sa_param[0] for _sa_param in _sa_parametrize], full_param_set, strict=False):
            if not argname:
...
def pytest_runtest_teardown(item, nextitem):
    ...
    if _current_class is not None and (
        ...
        nextitem is None or nextitem.getparent(pytest.Class) is not _current_class
    ):
        _current_class = None
