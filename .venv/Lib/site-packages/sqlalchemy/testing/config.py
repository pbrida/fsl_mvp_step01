...
def variation(argname_or_fn, cases=None):
    ...
    cases_plus_limitations = [
        (entry if (isinstance(entry, tuple) and len(entry) == 2) else (entry, None)) for entry in cases
    ]

    variations = Variation.generate_cases(argname, [c for c, l in cases_plus_limitations])
    return combinations(
        *[
            ((variation._name, variation, limitation) if limitation is not None else (variation._name, variation))
            for variation, (case, limitation) in zip(variations, cases_plus_limitations, strict=False)
        ],
        id_="ia",
        argnames=argname,
    )
...
class Config:
    def __init__(self, db, db_opts, options, file_config):
        ...
        self.is_async = db.dialect.is_async and not util.asbool(db.url.query.get("async_fallback", False))
