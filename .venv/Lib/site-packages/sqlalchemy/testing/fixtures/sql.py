        ...
        if self.run_define_tables != "each" and self.run_create_tables != "each" and self.run_deletes == "each":
            with self.bind.begin() as conn:
                for table in reversed(
                    [
                        t
                        for (t, fks) in sort_tables_and_constraints(self._tables_metadata.tables.values())
                        if t is not None
                    ]
                ):
                    try:
    ...
    def _load_fixtures(cls):
        ...
        for table, fks in sort_tables_and_constraints(cls._tables_metadata.tables.values()):
            ...
            with cls.bind.begin() as conn:
                conn.execute(
                    table.insert(),
                    [dict(zip(headers[table], column_values, strict=False)) for column_values in rows[table]],
                )
