...
class CollateTest(fixtures.TablesTest):
    ...
    def test_collate_order_by(self):
        ...
        self._assert_result(
            select(self.tables.some_table).order_by(self.tables.some_table.c.data.collate(collation).asc()),
...
class FetchLimitOffsetTest(fixtures.TablesTest):
    ...
    def test_simple_limit_expr_offset(self, connection):
        ...
        self._assert_result(
            connection,
            select(table).order_by(table.c.id).limit(2).offset(literal_column("1") + literal_column("1")),
            [(3, 3, 4), (4, 4, 5)],
        )

        self._assert_result(
            connection,
            select(table).order_by(table.c.id).limit(3).offset(literal_column("1") + literal_column("1")),
            [(3, 3, 4), (4, 4, 5), (5, 4, 6)],
        )
    ...
    def test_expr_limit_simple_offset(self, connection):
        ...
        self._assert_result(
            connection,
            select(table).order_by(table.c.id).limit(literal_column("1") + literal_column("1")).offset(2),
            [(3, 3, 4), (4, 4, 5)],
        )

        self._assert_result(
            connection,
            select(table).order_by(table.c.id).limit(literal_column("1") + literal_column("1")).offset(1),
            [(2, 2, 3), (3, 3, 4)],
        )
    ...
    def test_execute_tuple_expanding_plus_literal_execute(self):
        ...
        asserter.assert_(
            CursorSQL(
                ...
                "IN (%s(5, 10), (12, 18))" % ("VALUES " if config.db.dialect.tuple_in_values else ""),
                () if config.db.dialect.positional else {},
            )
        )
    ...
    def test_execute_tuple_expanding_plus_literal_heterogeneous_execute(self):
        ...
        asserter.assert_(
            CursorSQL(
                ...
                "IN (%s(5, 'z1'), (12, 'z3'))" % ("VALUES " if config.db.dialect.tuple_in_values else ""),
                () if config.db.dialect.positional else {},
            )
        )
...
class ExistsTest(fixtures.TablesTest):
    ...
    def test_select_exists(self, connection):
        ...
        eq_(
            connection.execute(select(literal(1)).where(exists().where(stuff.c.data == "some data"))).fetchall(),
            [(1,)],
        )
    ...
    def test_select_exists_false(self, connection):
        stuff = self.tables.stuff
        eq_(
            connection.execute(select(literal(1)).where(exists().where(stuff.c.data == "no data"))).fetchall(),
            [],
        )
...
class DistinctOnTest(AssertsCompiledSQL, fixtures.TablesTest):
    ...
    def test_distinct_on(self):
        ...
        with testing.expect_deprecated("DISTINCT ON is currently supported only by the PostgreSQL "):
