...
from ... import ForeignKey, Identity, Integer, MetaData, String, event, func, inspect, testing
...
from .. import config, engines, eq_, expect_raises, expect_raises_message, expect_warnings, fixtures, is_
    ...
    def define_temp_tables(cls, metadata):
        ...
        if testing.requires.view_reflection.enabled and testing.requires.temporary_views.enabled:
            event.listen(
                user_tmp,
                "after_create",
                DDL("create temporary view user_tmp_v as " "select * from user_tmp_%s" % config.ident),
            )
        ...
        if testing.requires.view_column_reflection.enabled:
            ...
            for name in names:
                ...
                event.listen(
                    metadata,
                    "before_drop",
                    DDL("DROP VIEW %s" % config.db.dialect.identifier_preparer.quote("view %s" % name)),
                )
...
class ComponentReflectionTest(ComparesTables, OneConnectionTablesTest):
    ...
    def _resolve_names(self, schema, scope, filter_names, values):
        ...
        res = {
            ...
            if scope_filter(k) and k[1] not in removed[schema] and (not filter_names or k[1] in filter_names)
        }
        return res
    ...
    def exp_pks(
        ...
        tables = {
            ...
            (schema, "email_addresses"): pk("address_id", name="email_ad_pk", comment="ea pk comment"),
    ...
    def exp_indexes(
        ...
        tables = {
            ...
            (schema, "noncol_idx_test_pk"): [*idx("q", name="noncol_idx_pk", column_sorting={"q": ("desc",)})],
            (schema, self.temp_table_name()): [
    ...
    def test_get_unique_constraints(self, metadata, connection, use_schema):
        ...
        uq_names = {uq.name for uq in reflected.constraints if isinstance(uq, sa.UniqueConstraint)}.difference(
            ["unique_c_a_b"]
        )
