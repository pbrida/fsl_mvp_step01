...
from typing import TYPE_CHECKING, Any, Callable, Deque, Dict, List, Optional, Tuple, Union, cast
...
class _ConnectionRecord(ConnectionPoolEntry):
    ...
    @util.deprecated(
        "2.0",
        "The _ConnectionRecord.connection attribute is deprecated; " "please use 'driver_connection'",
    )
...
def _finalize_fairy(
    ...
    pool: Pool,
    ref: Optional[weakref.ref[_ConnectionFairy]],  # this is None when called directly, not by the gc
...
class _ConnectionFairy(PoolProxiedConnection):
    ...
    @util.deprecated(
        "2.0",
        "The _ConnectionFairy.connection attribute is deprecated; " "please use 'driver_connection'",
    )
    def connection(self) -> DBAPIConnection:
        return self.dbapi_connection
        ...
        while attempts > 0:
            ...
            try:
                if pool._pre_ping:
                    if not connection_is_fresh:
                        ...
                        if not result:
                            if fairy._echo:
                                pool.logger.debug(
                                    "Pool pre-ping on connection %s failed, " "will invalidate pool",
                                    fairy.dbapi_connection,
                                )
                ...
                if e.invalidate_pool:
                    ...
                    pool.logger.info(
                        "Disconnection detected on checkout, " "invalidating individual connection %s (reason: %r)",
                        fairy.dbapi_connection,
                        e,
                    )
                    fairy._connection_record.invalidate(e)
