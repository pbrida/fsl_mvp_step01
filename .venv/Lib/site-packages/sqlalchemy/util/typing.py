...
_AnnotationScanType = Union[Type[Any], str, ForwardRef, NewType, TypeAliasType, "GenericProtocol[Any]"]
...
def eval_expression(
    ...
    *,
    ...
    try:
        ...
        raise NameError(
            f"Module {module_name} isn't present in sys.modules; can't " f"evaluate expression {expression}"
        ) from ke
...
def eval_name_only(
    ...
    *,
    ...
    try:
        ...
        raise NameError(f"Module {module_name} isn't present in sys.modules; can't " f"resolve name {name}") from ke
...
def is_origin_of(type_: Any, *names: str, module: Optional[str] = None) -> bool:
    ...
    return _get_type_name(origin) in names and (module is None or origin.__module__.startswith(module))
