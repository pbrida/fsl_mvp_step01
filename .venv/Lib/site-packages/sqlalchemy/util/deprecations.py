...
import re
from typing import Any, Callable, Dict, Match, Optional, Sequence, Set, Tuple, Type, TypeVar, Union

from .. import exc
from . import compat
from .langhelpers import _hash_limit_string, _warnings_warn, decorator, inject_docstring_text, inject_param_text
...
def became_legacy_20(api_name: str, alternative: Optional[str] = None, **kw: Any) -> Callable[[_F], _F]:
    ...
    if type_reg:
        ...
        type_ = "construct"
    message = "The %s %s is considered legacy as of the " "1.x series of SQLAlchemy and %s in 2.0." % (
        api_name,
        type_,
    ...
    def decorate(fn: _F) -> _F:
        ...
        if doc:
            doc = inject_param_text(
                doc,
                {
                    param: ".. deprecated:: %s %s" % ("1.4" if version == "2.0" else version, (message or ""))
                    for param, (version, message) in specs.items()
                },
            )
        decorated = warned(fn)
