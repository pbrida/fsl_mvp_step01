...
class PGCompiler(compiler.SQLCompiler):
    ...
    def visit_match_op_binary(self, binary, operator, **kw):
        if "postgresql_regconfig" in binary.modifiers:
            regconfig = self.render_literal_value(binary.modifiers["postgresql_regconfig"], sqltypes.STRINGTYPE)
            if regconfig:
    ...
    def visit_empty_set_expr(self, element_types, **kw):
        ...
        return "SELECT %s WHERE 1!=1" % (
            ", ".join(
                "CAST(NULL AS %s)" % self.dialect.type_compiler_instance.process(INTEGER() if type_._isnull else type_)
    ...
    def get_select_precolumns(self, select, **kw):
        ...
        if select._distinct or select._distinct_on:
            if select._distinct_on:
                return "DISTINCT ON (" + ", ".join([self.process(col, **kw) for col in select._distinct_on]) + ") "
            else:
                return "DISTINCT "
        else:
            return ""
    ...
    def delete_extra_from_clause(self, delete_stmt, from_table, extra_froms, from_hints, **kw):
        ...
        kw["asfrom"] = True
        return "USING " + ", ".join(t._compiler_dispatch(self, fromhints=from_hints, **kw) for t in extra_froms)
...
class PGDDLCompiler(compiler.DDLCompiler):
    def get_column_specification(self, column, **kwargs):
        ...
        if (
            ...
            and (self.dialect.supports_smallserial or not isinstance(impl_type, sqltypes.SmallInteger))
    ...
    def visit_create_enum_type(self, create, **kw):
        ...
        return "CREATE TYPE %s AS ENUM (%s)" % (
            ...
            ", ".join(self.sql_compiler.process(sql.literal(e), literal_binds=True) for e in type_.enums),
        )
    ...
    def visit_create_index(self, create, **kw):
            ...
            ", ".join(
                [
                    self.sql_compiler.process(
                        (expr.self_group() if not isinstance(expr, expression.ColumnClause) else expr),
                        include_table=False,
                        literal_binds=True,
                    )
        ...
        if withclause:
            text += " WITH (%s)" % (
                ", ".join(["%s = %s" % storage_parameter for storage_parameter in withclause.items()])
            )
    ...
    def visit_exclude_constraint(self, constraint, **kw):
        ...
        for expr, name, op in constraint._render_exprs:
            exclude_element = self.sql_compiler.process(expr, **kw) + (
                (" " + constraint.ops[expr.key]) if hasattr(expr, "key") and expr.key in constraint.ops else ""
            )
    ...
    def _can_comment_on_constraint(self, ddl_instance):
        ...
        if constraint.table is None:
            raise exc.CompileError(
                f"Can't emit COMMENT ON for constraint {constraint!r}: " "it has no associated table"
            )
    ...
    def _split_multihost_from_url(
        ...
        if "host" in url.query:
            if isinstance(url.query["host"], (list, tuple)):
                ...
                hosts, ports_str = zip(
                    *[token.split(":") if ":" in token else (token, None) for token in url.query["host"]], strict=False
                )
    ...
    def _constraint_query(self, is_unique):
        ...
        if is_unique:
            if self.server_version_info >= (15,):
                ...
                ).add_columns(sql.func.bool_and(pg_catalog.pg_index.c.indnullsnotdistinct).label("indnullsnotdistinct"))
            else:
                ...
                for row in result_by_oid[oid]:
                    ...
                    if any(idx_elements_is_expr):
                        index["column_names"] = [
                            ...
                            for expr, is_expr in zip(idx_elements, idx_elements_is_expr, strict=False)
                        ]
    ...
    def get_multi_check_constraints(self, connection, schema, filter_names, scope, kind, **kw):
        ...
        for table_name, check_name, src, comment in result:
            ...
            if not m:
                ...
                sqltext = re.compile(r"^[\s\n]*\((.+)\)[\s\n]*$", flags=re.DOTALL).sub(r"\1", m.group(1))
            entry = {
                "name": check_name,
                "sqltext": sqltext,
                "comment": comment,
    ...
    def _domain_query(self, schema):
        con_sq = (
            select(
                ...
                sql.func.array_agg(pg_catalog.pg_get_constraintdef(pg_catalog.pg_constraint.c.oid, True)).label(
                    "condefs"
                ),
                sql.func.array_agg(
                    # NOTE: cast since some postgresql derivatives may
