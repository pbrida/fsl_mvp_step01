...
class SQLiteExecutionContext(default.DefaultExecutionContext):
    ...
    def _preserve_raw_colnames(self):
        return not self.dialect._broken_dotted_colnames or self.execution_options.get("sqlite_raw_colnames", False)
...
class SQLiteDialect(default.DefaultDialect):
    ...
    def get_unique_constraints(self, connection, table_name, schema=None, **kw):
        ...
        def parse_uqs():
            ...
            INLINE_UNIQUE_PATTERN = r'(?:(".+?")|(?:[\[`])?([a-z0-9_]+)(?:[\]`])?) ' r"+[a-z0-9_ ]+? +UNIQUE"
    ...
    def get_indexes(self, connection, table_name, schema=None, **kw):
        ...
        for row in pragma_indexes:
            ...
            if len(row) >= 5 and row[4]:
                s = "SELECT sql FROM %(schema)ssqlite_master " "WHERE name = ? " "AND type = 'index'" % {
                    "schema": schema_expr
                }
    ...
    def _get_table_sql(self, connection, table_name, schema=None, **kw):
        ...
        try:
            ...
            s = "SELECT sql FROM %(schema)ssqlite_master " "WHERE name = ? " "AND type in ('table', 'view')" % {
                "schema": schema_expr
            }
            rs = connection.exec_driver_sql(s, (table_name,))
        value = rs.scalar()
