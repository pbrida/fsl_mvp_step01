...
from ... import Identity, Sequence, exc, sql, text, util
from ... import schema as sa_schema
from ...engine import cursor as _cursor
from ...engine import default, reflection
from ...engine.reflection import ReflectionDefaults
from ...sql import coercions, compiler, elements, expression, func, quoted_name, roles, sqltypes
        ...
        if self.isinsert:
            ...
            if id_column is not None and (not isinstance(id_column.default, Sequence)):
                ...
                self._enable_identity_insert = (id_column.key in self.compiled_parameters[0]) or (
                    compile_state._dict_parameters and (id_column.key in compile_state._insert_col_keys)
                )
            ...
            if self._enable_identity_insert:
                self.root_connection._cursor_execute(
                    self.cursor,
                    self._opt_encode("SET IDENTITY_INSERT %s ON" % self.identifier_preparer.format_table(tbl)),
                    (),
                    self,
                )
...
class MSSQLCompiler(compiler.SQLCompiler):
    ...
    def _use_top(self, select):
        return (select._offset_clause is None) and (
            ...
            or (
                ...
                and (select._fetch_clause_options["percent"] or select._fetch_clause_options["with_ties"])
            )
        )
    ...
    def _check_can_use_fetch_limit(self, select):
        ...
        if select._fetch_clause_options is not None and (
            ...
            raise exc.CompileError(
                "MSSQL needs TOP to use PERCENT and/or WITH TIES. " "Only simple fetch without offset can be used."
            )
    ...
    def translate_select_structure(self, select_stmt, **kwargs):
        ...
        if (
            ...
            select = (
                select.add_columns(sql.func.ROW_NUMBER().over(order_by=_order_by_clauses).label("mssql_rn"))
                .order_by(None)
                .alias()
            )
    ...
    def returning_clause(self, stmt, returning_cols, *, populate_result_map, **kw):
        ...
        columns = [
            ...
            for (
                ...
                column,
                repeated,
            ) in stmt._generate_columns_plus_names(True, cols=expression._select_iterables(returning_cols))
        ]
...
class MSDDLCompiler(compiler.DDLCompiler):
    ...
    def visit_create_index(self, create, include_schema=False, **kw):
        ...
        if index.dialect_options["mssql"]["include"]:
            inclusions = [
                index.table.c[col] if isinstance(col, str) else col for col in index.dialect_options["mssql"]["include"]
            ]
    ...
    def visit_set_table_comment(self, create, **kw):
        ...
        return "execute sp_addextendedproperty 'MS_Description', " "{}, 'schema', {}, 'table', {}".format(
    ...
    def visit_drop_table_comment(self, drop, **kw):
        ...
        schema_name = schema if schema else self.dialect.default_schema_name
        return "execute sp_dropextendedproperty 'MS_Description', 'schema', " "{}, 'table', {}".format(
            self.preparer.quote_schema(schema_name),
    ...
    def visit_set_column_comment(self, create, **kw):
        ...
        schema_name = schema if schema else self.dialect.default_schema_name
        return "execute sp_addextendedproperty 'MS_Description', " "{}, 'schema', {}, 'table', {}, 'column', {}".format(
            self.sql_compiler.render_literal_value(create.element.comment, sqltypes.NVARCHAR()),
            self.preparer.quote_schema(schema_name),
    ...
    def visit_drop_column_comment(self, drop, **kw):
        ...
        schema_name = schema if schema else self.dialect.default_schema_name
        return "execute sp_dropextendedproperty 'MS_Description', 'schema', " "{}, 'table', {}, 'column', {}".format(
            self.preparer.quote_schema(schema_name),
...
def _switch_db(dbname, connection, fn, *arg, **kw):
    if dbname:
        ...
        if current_db != dbname:
            connection.exec_driver_sql("use %s" % connection.dialect.identifier_preparer.quote(dbname))
    try:
        return fn(*arg, **kw)
    finally:
        if dbname and current_db != dbname:
            connection.exec_driver_sql("use %s" % connection.dialect.identifier_preparer.quote(current_db))
...
class MSDialect(default.DefaultDialect):
    ...
    def __init__(
        ...
        **opts,
        ...
        if legacy_schema_aliasing is not None:
            util.warn_deprecated(
                "The legacy_schema_aliasing parameter is " "deprecated and will be removed in a future release.",
                "1.4",
            )
    ...
    def get_isolation_level(self, dbapi_connection):
        ...
        try:
            cursor.execute(
                ("SELECT name FROM {} WHERE name IN " "('dm_exec_sessions', 'dm_pdw_nodes_exec_sessions')").format(
                    view_name
                )
            )
            row = cursor.fetchone()
            ...
            raise NotImplementedError(
                "Can't fetch isolation level;  encountered error {} when " 'attempting to query the "{}" view.'.format(
                    err, view_name
                )
            ) from err
        else:
    ...
    def _setup_version_attributes(self):
        if self.server_version_info[0] not in list(range(8, 17)):
            util.warn(
                ...
                "features may not function properly." % ".".join(str(x) for x in self.server_version_info)
            )
    ...
    def get_indexes(self, connection, tablename, dbname, owner, schema, **kw):
        filter_definition = (
            "ind.filter_definition" if self.server_version_info >= MS_2008_VERSION else "NULL as filter_definition"
        )
