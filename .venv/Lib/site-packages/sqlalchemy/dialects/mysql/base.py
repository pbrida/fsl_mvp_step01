...
from ... import exc, literal_column, log, sql, util
from ... import schema as sa_schema
from ...engine import cursor as _cursor
from ...engine import default, reflection
from ...engine.reflection import ReflectionDefaults
from ...sql import coercions, compiler, elements, functions, operators, roles, sqltypes, visitors
...
class MySQLExecutionContext(default.DefaultExecutionContext):
    def post_exec(self):
        if self.isdelete and cast(SQLCompiler, self.compiled).effective_returning and not self.cursor.description:
            ...
            self.cursor_fetch_strategy = _cursor.FullyBufferedCursorFetchStrategy(
                self.cursor,
                [(entry.keyname, None) for entry in cast(SQLCompiler, self.compiled)._result_columns],
                [],
            )
...
class MySQLCompiler(compiler.SQLCompiler):
    ...
    def visit_on_duplicate_key_update(self, on_duplicate, **kw):
        ...
        if on_duplicate._parameter_ordering:
            parameter_ordering = [
                coercions.expect(roles.DMLColumnRole, key) for key in on_duplicate._parameter_ordering
            ]
            ordered_keys = set(parameter_ordering)
            cols = [statement.table.c[key] for key in parameter_ordering if key in statement.table.c] + [
            ...
            if coercions._is_literal(val):
                ...
                def replace(obj):
                    if isinstance(obj, elements.BindParameter) and obj.type._isnull:
                        ...
                        if requires_mysql8_alias:
                            column_literal_clause = f"{_on_dup_alias_name}." f"{self.preparer.quote(obj.name)}"
                        else:
    ...
    def visit_cast(self, cast, **kw):
        ...
        if type_ is None:
            util.warn(
                ...
                "the CAST will be skipped." % self.dialect.type_compiler_instance.process(cast.typeclause.type)
            )
    ...
    def delete_extra_from_clause(self, delete_stmt, from_table, extra_froms, from_hints, **kw):
        ...
        kw["asfrom"] = True
        return "USING " + ", ".join(
            t._compiler_dispatch(self, fromhints=from_hints, **kw) for t in [from_table] + extra_froms
        )
...
class MySQLTypeCompiler(compiler.GenericTypeCompiler):
    ...
    def visit_NUMERIC(self, type_, **kw):
        if type_.precision is None:
            ...
            return self._extend_numeric(
                type_,
                "NUMERIC(%(precision)s, %(scale)s)" % {"precision": type_.precision, "scale": type_.scale},
            )
    ...
    def visit_DECIMAL(self, type_, **kw):
        if type_.precision is None:
            ...
            return self._extend_numeric(
                type_,
                "DECIMAL(%(precision)s, %(scale)s)" % {"precision": type_.precision, "scale": type_.scale},
            )
    ...
    def visit_DOUBLE(self, type_, **kw):
        if type_.precision is not None and type_.scale is not None:
            return self._extend_numeric(
                type_,
                "DOUBLE(%(precision)s, %(scale)s)" % {"precision": type_.precision, "scale": type_.scale},
            )
        else:
    ...
    def visit_REAL(self, type_, **kw):
        if type_.precision is not None and type_.scale is not None:
            return self._extend_numeric(
                type_,
                "REAL(%(precision)s, %(scale)s)" % {"precision": type_.precision, "scale": type_.scale},
            )
        else:
    ...
    def _set_mariadb(self, is_mariadb, server_version_info):
        ...
        if not is_mariadb and self.is_mariadb:
            raise exc.InvalidRequestError(
                "MySQL version %s is not a MariaDB variant." % (".".join(map(str, server_version_info)),)
            )
        if is_mariadb:
