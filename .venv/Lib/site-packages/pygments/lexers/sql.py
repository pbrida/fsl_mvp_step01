    ...
    tokens = {
        "root": [
            ...
            (
                r"("
                + "|".join(
                    s.replace(" ", r"\s+") for s in _postgres_builtins.DATATYPES + _postgres_builtins.PSEUDO_TYPES
                )
                + r")\b",
                Name.Builtin,
            ),
...
class PlPgsqlLexer(PostgresBase, RegexLexer):
    ...
    for i, pattern in enumerate(tokens["root"]):
        if pattern[1] == Keyword:
            tokens["root"][i] = (
                words(_postgres_builtins.KEYWORDS + _postgres_builtins.PLPGSQL_KEYWORDS, suffix=r"\b"),
                Keyword,
            )
            del i
            break
