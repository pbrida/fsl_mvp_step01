...
class MultipleTyperAnnotationsError(Exception):
    ...
    def __str__(self) -> str:
        return "Cannot specify multiple `Annotated` Typer arguments" f" for {self.argument_name!r}"
...
class DefaultFactoryAndDefaultValueError(Exception):
    ...
    def __str__(self) -> str:
        param_type_str = _param_type_to_user_string(self.param_type)
        return "Cannot specify `default_factory` and a default value together" f" for {param_type_str}"
...
def _split_annotation_from_typer_annotations(
    ...
    return base_annotation, [
        annotation for annotation in maybe_typer_annotations if isinstance(annotation, ParameterInfo)
    ]
