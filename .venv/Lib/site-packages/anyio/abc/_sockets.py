    ...
    *,
    ...
    try:
        ...
        if addr_family != socket.AF_UNSPEC and sock.family != addr_family:
            raise ValueError(f"address family mismatch: expected {addr_family.name}, got " f"{sock.family.name}")
...
class UNIXSocketStream(SocketStream):
    ...
    async def from_socket(cls, sock_or_fd: socket.socket | int) -> UNIXSocketStream:
        ...
        """
        sock = _validate_socket(sock_or_fd, socket.SOCK_STREAM, socket.AF_UNIX, require_connected=True)
...
class ConnectedUNIXDatagramSocket(UnreliableObjectStream[bytes], _SocketProvider):
    ...
    async def from_socket(
        ...
        """
        sock = _validate_socket(sock_or_fd, socket.SOCK_DGRAM, socket.AF_UNIX, require_connected=True)
