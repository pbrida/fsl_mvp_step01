...
def is_anyio_cancellation(exc: CancelledError) -> bool:
    ...
    while True:
        if exc.args and isinstance(exc.args[0], str) and exc.args[0].startswith("Cancelled by cancel scope "):
            return True
...
class CancelScope(BaseCancelScope):
    ...
    def _parent_cancellation_is_visible_to_us(self) -> bool:
        return self._parent_scope is not None and not self.shield and self._parent_scope._effectively_cancelled
        ...
        try:
            ...
            try:
                        ...
                        while self._tasks:
                            ...
                            try:
                                ...
                                if exc_val is None or (
                                    isinstance(exc_val, CancelledError) and not is_anyio_cancellation(exc)
                                ):
                                    exc_val = exc
                ...
                if self._exceptions:
                    ...
                    raise BaseExceptionGroup("unhandled errors in a TaskGroup", self._exceptions) from None
                elif exc_val:
                    raise exc_val
...
class AsyncIOBackend(AsyncBackend):
    ...
    async def wrap_udp_socket(cls, sock: socket.socket) -> UDPSocket:
        transport, protocol = await get_running_loop().create_datagram_endpoint(DatagramProtocol, sock=sock)
        return UDPSocket(transport, protocol)
    ...
    @classmethod
    async def wrap_connected_unix_datagram_socket(cls, sock: socket.socket) -> ConnectedUNIXDatagramSocket:
    ...
    @classmethod
    def open_signal_receiver(cls, *signals: Signals) -> AbstractContextManager[AsyncIterator[Signals]]:
