...
class MessageExtractor:
    ...
    def process_file(self, fileobj):
        template_node = lexer.Lexer(fileobj.read(), input_encoding=self.config["encoding"]).parse()
    ...
    def extract_nodes(self, nodes):
        ...
        for node in nodes:
            child_nodes = None
            if in_translator_comments and isinstance(node, parsetree.Text) and not node.content.strip():
            ...
            if translator_comments and translator_comments[-1][0] < node.lineno - 1:
                translator_comments = []
            ...
            for message in self.process_python(code, node.lineno - 1, translator_strings):
                yield message
