...
def split_email(email: str) -> Tuple[Optional[str], str, str, bool]:
    ...
    if right_part.startswith("<"):
        ...
        if not display_name_quoted:
            bad_chars = {
                safe_character_display(c) for c in display_name if (not ATEXT_RE.match(c) and c != " ") or c == "."
            }
            if bad_chars:
                raise EmailSyntaxError(
...
def validate_email_local_part(
    ...
    if bad_chars:
        raise EmailSyntaxError(
            "The email address contains invalid characters before the @-sign: " + ", ".join(sorted(bad_chars)) + "."
        )
...
def uts46_valid_char(char: str) -> bool:
    ...
    if 0x80 <= c <= 0x9F:
        ...
        (0x2010 <= c <= 0x2060 and not (0x2024 <= c <= 0x2026) and not (0x2028 <= c <= 0x202E))
...
def validate_email_domain_name(
    ...
    bad_chars = {safe_character_display(c) for c in domain if not ATEXT_HOSTNAME_INTL.match(c)}
    if bad_chars:
        raise EmailSyntaxError(
            "The part after the @-sign contains invalid characters: " + ", ".join(sorted(bad_chars)) + "."
        )
    ...
    if DOT_ATOM_TEXT_HOSTNAME.match(domain):
        ...
        try:
            ascii_domain = ".".join(idna.alabel(label).decode("ascii") for label in domain.split("."))
        except idna.IDNAError as e:
    ...
    # case for this.
    bad_chars = {safe_character_display(c) for c in domain_i18n if not ATEXT_HOSTNAME_INTL.match(c)}
    if bad_chars:
        raise EmailSyntaxError(
            "The part after the @-sign contains invalid characters: " + ", ".join(sorted(bad_chars)) + "."
        )
...
def validate_email_length(addrinfo: ValidatedEmail) -> None:
    ...
    addresses_to_check = [
        ...
        (
            (addrinfo.ascii_local_part or addrinfo.local_part or "") + "@" + addrinfo.ascii_domain,
...
def validate_email_domain_literal(domain_literal: str) -> DomainLiteralValidationResult:
    ...
    bad_chars = {safe_character_display(c) for c in domain_literal if not DOMAIN_LITERAL_CHARS.match(c)}
    if bad_chars:
        raise EmailSyntaxError(
            "The part after the @-sign contains invalid characters in brackets: " + ", ".join(sorted(bad_chars)) + "."
        )
