...
def validate_email(
    ...
    *,  # subsequent arguments are keyword-only
    ...
    ret = ValidatedEmail()
    ret.original = (
        (local_part if not is_quoted_local_part else ('"' + local_part + '"')) + "@" + domain_part
    ...
    if len(domain_part) == 0:
        ...
        domain_name_info = validate_email_domain_name(
            domain_part, test_environment=test_environment, globally_deliverable=globally_deliverable
        )
    ...
    if check_deliverability and not test_environment:
        ...
        deliverability_info = validate_email_deliverability(ret.ascii_domain, ret.domain, timeout, dns_resolver)
