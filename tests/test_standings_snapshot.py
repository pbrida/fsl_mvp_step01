def test_standings_snapshot_after_simulation(client):
    ...
    # League + 2 teams
    r = client.post("/leagues/", json={"name": "Snapshot League", "roster_slots": 14, "starters": 8, "bucket_requirements": {"X": 8}})
    assert r.status_code == 200
    league_id = r.json()["id"]
    ...
    starters_beta  = ["AA","BB","CC","DD","EE","ETF1","FX1","FX2"]  # same symbols -> same 55
    # Make Beta weaker by turning two symbols to zero-projection placeholders
    r = client.post("/players/seed", json=[
        {"symbol": "ZZ1", "name": "ZZ1", "is_etf": False, "market_cap": 1e9, "primary_bucket": "SMALL_CAP", "proj_points": 0.0},
        {"symbol": "ZZ2", "name": "ZZ2", "is_etf": False, "market_cap": 1e9, "primary_bucket": "LARGE_CAP", "proj_points": 0.0},
    ])
    assert r.status_code == 200
