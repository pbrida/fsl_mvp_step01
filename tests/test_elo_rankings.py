...
def seed_players_for_elo(client):
    ...
    r = client.post("/players/seed", json=[
        ...
        {"symbol": "C1", "name":"C1",  "is_etf": False, "market_cap": 2e11, "primary_bucket": "LARGE_CAP", "proj_points": 2.0},
        {"symbol": "C2", "name":"C2",  "is_etf": False, "market_cap": 5e9,  "primary_bucket": "MID_CAP",   "proj_points": 2.0},
        {"symbol": "C3", "name":"C3",  "is_etf": False, "market_cap": 1e9,  "primary_bucket": "SMALL_CAP", "proj_points": 2.0},
        {"symbol": "C4", "name":"C4",  "is_etf": False, "market_cap": 1e11, "primary_bucket": "ETF",       "proj_points": 2.0},
        {"symbol": "C5", "name":"C5",  "is_etf": False, "market_cap": 2e11, "primary_bucket": "LARGE_CAP", "proj_points": 1.0},
        {"symbol": "C6", "name":"C6",  "is_etf": False, "market_cap": 1e9,  "primary_bucket": "SMALL_CAP", "proj_points": 1.0},
        {"symbol": "C7", "name":"C7",  "is_etf": False, "market_cap": 1e9,  "primary_bucket": "SMALL_CAP", "proj_points": 0.5},
        {"symbol": "C8", "name":"C8",  "is_etf": False, "market_cap": 1e9,  "primary_bucket": "SMALL_CAP", "proj_points": 0.5},
    ])
...
def test_elo_rankings_flow(client):
    ...
    # League + 3 teams
    r = client.post("/leagues/", json={"name": "EloLeague", "roster_slots": 14, "starters": 8, "bucket_requirements": {"X": 8}})
    assert r.status_code == 200
    league_id = r.json()["id"]

    rr = client.post(f"/leagues/{league_id}/join", json={"name": "Alpha", "owner": "A"}); assert rr.status_code == 200
    t_alpha = rr.json()["id"]
    rr = client.post(f"/leagues/{league_id}/join", json={"name": "Beta", "owner": "B"});  assert rr.status_code == 200
    t_beta  = rr.json()["id"]
    rr = client.post(f"/leagues/{league_id}/join", json={"name": "Gamma", "owner": "C"}); assert rr.status_code == 200
    t_gamma = rr.json()["id"]
