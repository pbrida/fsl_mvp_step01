...
def test_team_needs_primary_then_flex(client):
    ...
    r = client.post("/players/seed", json=[
        {"symbol": "AAPL", "name": "Apple", "is_etf": False, "market_cap": 2.5e12, "primary_bucket": "LARGE_CAP"},
        {"symbol": "GOOGL","name": "Alphabet","is_etf": False, "market_cap": 1.8e12, "primary_bucket": "LARGE_CAP"},
        {"symbol": "VTI",  "name": "Vanguard Total", "is_etf": True,  "market_cap": 3e11, "primary_bucket": "ETF"},
        {"symbol": "SHOP", "name": "Shopify","is_etf": False, "market_cap": 8e10, "primary_bucket": "SMALL_CAP"},
        {"symbol": "NEWM", "name": "New Mid", "is_etf": False, "market_cap": 5e9,  "primary_bucket": "MID_CAP"},
    ])
    assert r.status_code == 200

    # League + team
    r = client.post("/leagues/", json={"name": "Needs League", "roster_slots": 14, "starters": 8, "bucket_requirements": {"X": 8}})
    assert r.status_code == 200
    league_id = r.json()["id"]
