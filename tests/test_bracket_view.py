...
def test_bracket_view_end_to_end(client):
    ...
    # League + 4 teams
    r = client.post("/leagues/", json={"name": "Bracket", "roster_slots": 14, "starters": 8, "bucket_requirements": {"X": 8}})
    assert r.status_code == 200
    league_id = r.json()["id"]
    ...
    r = client.get(f"/season/{league_id}/bracket"); assert r.status_code == 200
    b = r.json()
    ...
    r = client.get(f"/season/{league_id}/bracket"); b = r.json()
